...
func (p *itemRecorder) Publish(_ context.Context, subject string, env *events.Envelope, _ ...events.PublishOption) (events.Receipt, error) {
	if strings.HasPrefix(subject, "clustarr.evt.catalog.album.") {
		p.mu.Lock()
...
// TestAlbumReconcilerPublishesItemEvents: spec §5's
// clustarr.evt.catalog.album.<added|updated|deleted>.<uid>, once per edge --
...
func TestAlbumReconcilerPublishesItemEvents(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	cfg := newTestConfig(t)
	c := startCacheOnly(t, ctx, cfg)
	const ns = "album-itemevent-ns"
	require.NoError(t, c.Create(ctx, testNamespace(ns)))

	obj := &catalogv1alpha1.Album{ObjectMeta: metav1.ObjectMeta{Name: "ok-computer", Namespace: ns}, Spec: catalogv1alpha1.AlbumSpec{ArtistRef: "nobody", ReleaseGroupID: "rg-x"}}
	require.NoError(t, c.Create(ctx, obj))
	key := client.ObjectKeyFromObject(obj)
	require.Eventually(t, func() bool { return c.Get(ctx, key, &catalogv1alpha1.Album{}) == nil }, 5*time.Second, 10*time.Millisecond)

	pub := &itemRecorder{}
	r := &album.Reconciler{Client: c, Scheme: k8s.MustNewScheme(), Recorder: k8sevents.NewFakeRecorder(10), Bus: combinedBus{Publisher: pub, requester: fakeAlbumLookupRPC{}}}
	reconcileOnce := func() {
		t.Helper()
	...
	assert.Equal(t, events.CatalogItemSubject("album", events.ActionAdded, uid), subject)
	assert.Equal(t, uid+":added", env.ID)
	assert.Equal(t, events.ActionAdded, evt.Action)
	assert.Equal(t, commonv1.MediaRef{Kind: commonv1.MediaKindAlbum, Name: key.Name}, evt.Media)
	assert.Equal(t, schema.Ref{Namespace: ns, Name: key.Name, UID: uid}, evt.Ref)
	assert.Equal(t, map[string]string{"mb-release-group": "rg-x"}, evt.IDs)
	...
	assert.Equal(t, events.CatalogItemSubject("album", events.ActionUpdated, uid), subject)
	...
	assert.Equal(t, events.CatalogItemSubject("album", events.ActionDeleted, uid), subject)
