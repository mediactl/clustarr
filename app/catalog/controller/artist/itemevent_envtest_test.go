...
func (p *itemRecorder) Publish(_ context.Context, subject string, env *events.Envelope, _ ...events.PublishOption) (events.Receipt, error) {
	if strings.HasPrefix(subject, "clustarr.evt.catalog.artist.") {
		p.mu.Lock()
...
// TestArtistReconcilerPublishesItemEvents: spec §5's
// clustarr.evt.catalog.artist.<added|updated|deleted>.<uid>, once per edge --
...
func TestArtistReconcilerPublishesItemEvents(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	cfg := newTestConfig(t)
	c := startCacheOnly(t, ctx, cfg)
	const ns = "artist-itemevent-ns"
	...
	obj := &catalogv1alpha1.Artist{ObjectMeta: metav1.ObjectMeta{Name: "radiohead", Namespace: ns}, Spec: catalogv1alpha1.ArtistSpec{MusicBrainzID: "mb-x", QualityProfileRef: "none", RootFolderRef: "music-root"}}
	require.NoError(t, c.Create(ctx, obj))
	key := client.ObjectKeyFromObject(obj)
	require.Eventually(t, func() bool { return c.Get(ctx, key, &catalogv1alpha1.Artist{}) == nil }, 5*time.Second, 10*time.Millisecond)

	pub := &itemRecorder{}
	r := &artist.Reconciler{Client: c, Scheme: k8s.MustNewScheme(), Recorder: k8sevents.NewFakeRecorder(10), Bus: combinedBus{Publisher: pub, requester: fakeAlbumRPC{}}}
	reconcileOnce := func() {
		t.Helper()
	...
	assert.Equal(t, events.CatalogItemSubject("artist", events.ActionAdded, uid), subject)
	assert.Equal(t, uid+":added", env.ID)
	assert.Equal(t, events.ActionAdded, evt.Action)
	assert.Equal(t, commonv1.MediaRef{Kind: commonv1.MediaKindArtist, Name: key.Name}, evt.Media)
	assert.Equal(t, schema.Ref{Namespace: ns, Name: key.Name, UID: uid}, evt.Ref)
	assert.Equal(t, map[string]string{"mb-artist": "mb-x"}, evt.IDs)
	...
	assert.Equal(t, events.CatalogItemSubject("artist", events.ActionUpdated, uid), subject)
	...
	assert.Equal(t, events.CatalogItemSubject("artist", events.ActionDeleted, uid), subject)
