...
func (p *itemRecorder) Publish(_ context.Context, subject string, env *events.Envelope, _ ...events.PublishOption) (events.Receipt, error) {
	if strings.HasPrefix(subject, "clustarr.evt.catalog.author.") {
		p.mu.Lock()
...
// TestAuthorReconcilerPublishesItemEvents: spec §5's
// clustarr.evt.catalog.author.<added|updated|deleted>.<uid>, once per edge --
...
func TestAuthorReconcilerPublishesItemEvents(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	cfg := newTestConfig(t)
	c := startCacheOnly(t, ctx, cfg)
	const ns = "author-itemevent-ns"
	require.NoError(t, c.Create(ctx, testNamespace(ns)))
	createRootFolder(t, ctx, c, ns, "book-root", "/data/media/books")

	obj := &catalogv1alpha1.Author{ObjectMeta: metav1.ObjectMeta{Name: "le-guin", Namespace: ns}, Spec: catalogv1alpha1.AuthorSpec{OpenLibraryID: "OL7A", QualityProfileRef: "none", RootFolderRef: "book-root"}}
	require.NoError(t, c.Create(ctx, obj))
	key := client.ObjectKeyFromObject(obj)
	require.Eventually(t, func() bool { return c.Get(ctx, key, &catalogv1alpha1.Author{}) == nil }, 5*time.Second, 10*time.Millisecond)

	pub := &itemRecorder{}
	r := &author.Reconciler{Client: c, Scheme: k8s.MustNewScheme(), Recorder: k8sevents.NewFakeRecorder(10), Bus: combinedBus{Publisher: pub, requester: fakeBookListRPC{}}}
	reconcileOnce := func() {
		t.Helper()
	...
	assert.Equal(t, events.CatalogItemSubject("author", events.ActionAdded, uid), subject)
	assert.Equal(t, uid+":added", env.ID)
	assert.Equal(t, events.ActionAdded, evt.Action)
	assert.Equal(t, commonv1.MediaRef{Kind: commonv1.MediaKindAuthor, Name: key.Name}, evt.Media)
	assert.Equal(t, schema.Ref{Namespace: ns, Name: key.Name, UID: uid}, evt.Ref)
	assert.Equal(t, map[string]string{"olauthor": "OL7A"}, evt.IDs)
	...
	assert.Equal(t, events.CatalogItemSubject("author", events.ActionUpdated, uid), subject)
	...
	assert.Equal(t, events.CatalogItemSubject("author", events.ActionDeleted, uid), subject)
