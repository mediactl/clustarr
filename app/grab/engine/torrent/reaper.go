...
// that no longer exists -- and a transfer this engine re-attaches when it
// comes back then has no Download at all: a torrent goes on seeding with
...
// this replica's own watch delivers that write back to its cache). The age
// is the transfer's own [download.Item.AddedAt], which [Reconciler.add]
// persists in the re-attach descriptor and [Engine.ReAttach] hands back to
// the client, so an engine restart does not grant an orphan a fresh grace
// period ([engine.OrphanClock] has the details, and the fallback for a
// descriptor written before the age was recorded).
//
// # Forgetting is durable
//
// A reaped transfer's re-attach descriptor is removed with it. Leaving it
// would make the next restart re-add the orphan, which would then seed
// until the next reap -- after every restart.
//
...
func (r *Reaper) ReapOnce(ctx context.Context) error {
	...
	for _, id := range r.orphansDue(items, known) {
		...
		if err := removeDescriptor(r.Engine.StateDir, id); err != nil {
			log.ErrorContext(ctx, "torrent: orphan reap: remove descriptor failed", "id", id, "error", err)
		}
		r.clock.Forget(id)
