...
// decodes the JSON body into out. The caller is responsible for handling
// any other status code from the returned response; doJSON never treats a
// non-200 status as an error itself, since the callers in this package
// (device-flow polling in particular) attach meaning to specific non-200
// codes.
