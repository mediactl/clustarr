...
// same aspect. A badge stacks its logo above the score, the pair centred
// in the box both ways, the digits at the count's height.
...
func drawBadge(canvas *image.NRGBA, box image.Rectangle, paddingPx, radiusPx int, b Badge, t Template) error {
	...
	// The logo above the score, the pair centred in the box as one group,
	// as Plex centres its count -- the owner's choice over a single row
	// (2026-09-24), compared against a Plex item side by side.
	groupH := logoH + capH
	if logoH > 0 && capH > 0 {
		groupH += margin
	}
	...
	if face != nil {
		// Centred by the glyphs' ink, not their advance, whose side
		// bearings would put the score a few pixels left of centre.
		ink, _ := font.BoundString(face, b.Score)
		inkLeft := fixedToFloat(ink.Min.X)
		inkW := fixedToFloat(ink.Max.X) - inkLeft
		x := float64(box.Min.X) + (float64(boxW)-inkW)/2 - inkLeft
		drawScoreText(canvas, b.Score, face, x, float64(top+capH))
	}
	return nil
}
