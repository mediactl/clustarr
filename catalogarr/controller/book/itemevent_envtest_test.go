...
func (p *itemRecorder) Publish(_ context.Context, subject string, env *events.Envelope, _ ...events.PublishOption) (events.Receipt, error) {
	if strings.HasPrefix(subject, "clustarr.evt.catalog.book.") {
		p.mu.Lock()
...
// TestBookReconcilerPublishesItemEvents: spec §5's
// clustarr.evt.catalog.book.<added|updated|deleted>.<uid>, once per edge --
...
func TestBookReconcilerPublishesItemEvents(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	cfg := newTestConfig(t)
	c := startCacheOnly(t, ctx, cfg)
	const ns = "book-itemevent-ns"
	require.NoError(t, c.Create(ctx, testNamespace(ns)))

	obj := &catalogv1alpha1.Book{ObjectMeta: metav1.ObjectMeta{Name: "earthsea", Namespace: ns}, Spec: catalogv1alpha1.BookSpec{WorkID: "OL7W"}}
	require.NoError(t, c.Create(ctx, obj))
	key := client.ObjectKeyFromObject(obj)
	require.Eventually(t, func() bool { return c.Get(ctx, key, &catalogv1alpha1.Book{}) == nil }, 5*time.Second, 10*time.Millisecond)
	...
	assert.Equal(t, events.CatalogItemSubject("book", events.ActionAdded, uid), subject)
	assert.Equal(t, uid+":added", env.ID)
	assert.Equal(t, events.ActionAdded, evt.Action)
	assert.Equal(t, commonv1.MediaRef{Kind: commonv1.MediaKindBook, Name: key.Name}, evt.Media)
	assert.Equal(t, schema.Ref{Namespace: ns, Name: key.Name, UID: uid}, evt.Ref)
	assert.Equal(t, map[string]string{"olwork": "OL7W"}, evt.IDs)
	...
	assert.Equal(t, events.CatalogItemSubject("book", events.ActionUpdated, uid), subject)
	...
	assert.Equal(t, events.CatalogItemSubject("book", events.ActionDeleted, uid), subject)
