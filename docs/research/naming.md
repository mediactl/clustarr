...
Prioritisation (`DownloadDecisionComparer`, **verified** against `NzbDrone.Core/DecisionEngine/DownloadDecisionComparer.cs` in both apps during Phase C; `pkg/decision.Rank` implements it -- this line was "unverified order" and omitted two real steps until gap fix X13): quality (profile index, then revision/proper/repack, the revision only when the profile does not say doNotPrefer) -> custom format score -> preferred protocol from the delay profile -> **episode count** (Sonarr: prefer the pack when searching a season; Sonarr also compares episode number here) -> indexer priority (lower number wins, default 25) -> **indexer flags** (Radarr `ScoreFlags`: freeleech, doubleupload and internal +2 each, halfleech +1) -> seeders, `round(log10(seeders))` (torrent), or age bucket (usenet: under 1h, within 24h, within 7d, then by log10 of days) -> size (closest to the quality's preferred MB/min x runtime in 200 MiB buckets; the largest when the preferred size is the unlimited sentinel).
