...
- **M7 Index/artwork/ratings/Plex (done, 2026-09-24):** `pkg/relindex` gains a selectable Postgres engine (`--index-dsn`, CloudNativePG, ADR-0010); a JetStream object store (`clustarr-artwork`) holds provider and custom artwork, served by `ui` at `/art` (ADR-0011); `status.metadata.ratings` from TMDB, with MDBList and OMDb declared but not built (ruling R5, no API keys); `OverlayProfile` and the `catalogarr --role artwork` renderer composite Kometa-style rating badges onto posters; `ui`'s `/plex/movies` and `/plex/tv` serve Plex's Custom Metadata Provider protocol (ADR-0012). Design: `docs/superpowers/specs/2026-09-24-index-artwork-ratings-plex-design.md`.
