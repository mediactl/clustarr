...
**The lease protocol.** A worker claims a task with a compare-and-swap create in `clustarr-transcode-leases` (bucket TTL 90s, server-expired, so worker clock skew cannot matter), renews it while it encodes, and stops itself at least 30s before the lease could expire. squasharr writes a `cancelled` marker on withdrawal, scoped to that attempt and earlier ones only, so a job withdrawn and re-dispatched inside the lease TTL is never cancelled by its own predecessor's marker (design §6, §9).

**Results and the one CAS status writer.** The worker publishes `transcode.StatusEvent.v1` (`claimed`, `progress` at most every 10s, `finished`) to `clustarr.work.transcode.result.<jobUID>`, acking its task only once JetStream has durably stored `finished`. squasharr's static `squasharr-transcode-results` consumer (AckWait 30s, MaxDeliver 10, BackOff 5s/30s/2m, MaxAckPending 1) is the only other writer of `TranscodeJob.status` besides the reconciler, and both go through one function: read the object fresh through the uncached reader, change a copy of its status, apply it with the read `resourceVersion` as a precondition. A write that races the other path fails Conflict and is redone from a fresh read -- never silently rolled back. `claimed`/`progress` move a job to Running and set `workerPod`/`startedAt`/`progress`; an event with a stale `attempt`, or that reaches a terminal or undispatched job, is acked and changes nothing (design §18.1-§18.2).
