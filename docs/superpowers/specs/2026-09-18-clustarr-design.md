...
type DownloadClientSpec struct {
    ...
    Usenet *UsenetSpec    // req iff protocol==usenet. {Providers []NNTPProvider{Name, Host string; Port int32 =563; TLS bool =true; Connections int32; SecretRef corev1.LocalObjectReference; Backup bool; Priority int32; QuotaBytes *int64} (≥1); PostProcess{Par2 bool =true; Unpack bool =true; DeleteArchives bool =true; CleanupPatterns []string}; PropagationDelay metav1.Duration; PreCheck bool; AbortHealthPercent int32 =90; HealthAction enum{pause,delete} ="pause"; Scratch{SizeLimit resource.Quantity ="50Gi"; StorageClassName *string; VolumeName string; AccessModes []PersistentVolumeAccessMode; ExistingClaim string; Path string /* one placement: path on the data mount > existingClaim > a claim the controller makes (storageClassName or volumeName) > emptyDir; CEL keeps them exclusive; 2026-09-24 */}; PublishDir string /* "" = the data mount; <publishDir>/<category>/<name>; 2026-09-24 */; DownloadTimeout *metav1.Duration /* unset = no deadline; gap fix Y2 */}
...
type TranscodeJobStatus struct {
    ...
    Phase string enum{Pending,Planned,Queued,Running,Verifying,Succeeded,Failed,Skipped}  // Verifying: legacy, never set under worker pools (§6.4)
...
### 4.7 Artwork, ratings and OverlayProfile (M7, 2026-09-24)

`catalog.clustarr.io` gains `Artwork []ArtworkOverride` (spec) and
`Artwork []ArtworkEntry`/`Overlay *OverlayEntry` (status) on Movie, Series,
Artist, Album, Author, Book, Audiobook and Comic (`Overlay` on Movie and
Series only); `MovieMetadata` and `SeriesMetadata` gain `Ratings []Rating`,
and `SeriesMetadata` gains `FirstAired *metav1.Time`; `MetadataProviderType`
gains `mdblist` and `omdb`; and a new namespaced kind, `OverlayProfile`,
selects Movie and Series by label and drives the rating-badge renderer.
Field shapes, CEL and status budgets live in the M7 design
(`docs/superpowers/specs/2026-09-24-index-artwork-ratings-plex-design.md`
§B.6, §C.1, §C.4) rather than being restated here.
...
| `CLUSTARR_WORK_SQUASHARR` (transcode-worker-pools plan, spec 2026-09-23 §5) | `clustarr.work.transcode.>` | WorkQueue | – | 64 MiB | 1h | File, R3, DiscardNew (no `AllowMsgSchedules`: a task's Msg-Id already carries the dispatch attempt, so nothing needs a scheduled delay); the per-(profile, hardware class) pool consumer that reads it is created and deleted by the pool reconciler, one per running pool, and is **not** in `Default()` |
...
WorkQueue retention is immutable: `natsbus.Ensure` refuses to start if an existing stream's retention differs (operator must migrate). grabarr has no work stream. The four catalogarr/importarr/indexarr/captionarr work streams are **DiscardOld**, not the DiscardNew this table first asked for: nats-server refuses DiscardNew beside message schedules ("message scheduling cannot use discard new"), so those streams are sized with headroom that makes the limit an alarm, and publishers still handle `ErrQueueFull`. `CLUSTARR_WORK_SQUASHARR` carries no scheduled messages, so it keeps DiscardNew: a full stream hands the dispatcher `ErrQueueFull` rather than silently discarding the oldest undelivered task. This table matches `pkg/events.Default()` (gap-fix X13 brought it up to date, gap fix Y3 added `catalogarr-redownload`, gap fixes Z2 added `CLUSTARR_ADVISORIES` and pruned the `catalogarr-import` and `indexarr-definitions` consumers, their subjects and `index.DefinitionsSync`, and the `clustarr-search-cache` bucket, none of which anything produced, subscribed or read, and the transcode-worker-pools plan added `CLUSTARR_WORK_SQUASHARR`, its `squasharr-transcode-results` consumer and the `clustarr-transcode-leases` bucket: nine streams, fifteen consumers, eleven buckets in `Default()`, plus one pool consumer per running (profile, hardware class) outside it). `natsbus.Ensure` never deletes, so a broker created earlier keeps the two orphan consumers and the bucket, idle, until an operator removes them.
...
| `clustarr.work.transcode.task.<profileUID>.<class>.<jobUID>` (transcode-worker-pools plan, spec 2026-09-23 §5-§7) | `transcode.TranscodeTask.v1` | squasharr TranscodeJob controller, on admission, one durable pull consumer per (profile, hardware class) pool → the pool's `squasharr-worker` pods (`CLUSTARR_WORK_SQUASHARR`) |
| `clustarr.work.transcode.result.<jobUID>` (transcode-worker-pools plan, spec 2026-09-23 §5, §18.1-§18.2) | `transcode.TranscodeResult.v1` (`claimed`\|`progress`\|`finished`) | `squasharr-worker` → `squasharr-transcode-results` (squasharr TranscodeJob controller, the sole `TranscodeJob.status` writer, one compare-and-swap path) |
...
| `clustarr-progress` | `scan.<scan-uid>` (rescan checkpoint), `importlist.<list-uid>` (sync result), `download.<uid>` (`schema.DownloadProgress`, gap fixes Z1), `transcode.<uid>` (`schema.TranscodeProgress`, gap fixes Z5); every `<uid>` through `KVKeyToken` | 10m | importarr's checkpoints, read by its controllers; 1 Hz live telemetry for a UI that wants motion between status writes (the UI reads status today). The grabarr engines write a sample only when it changed, at most once a second, and delete the key when the transfer leaves the client; the `squasharr-worker` pool pod writes at most once a second, one Put in flight with a 2 s timeout, and lets the key expire. `NATS_URL` is no longer optional for this write: the transcode-worker-pools plan's worker has no Kubernetes access at all, so every pool pod carries it to reach the task queue in the first place, not only for telemetry. Neither write can fail or slow the transfer |
| `clustarr-transcode-leases` (transcode-worker-pools plan, spec 2026-09-23 §5-§6, new) | `<jobUID>` (`KVKeyToken`) → `{state: held\|cancelled, attempt, pod, node, since}` | 90s, history 1, server-expired | the fencing lease a worker creates on claiming a task and renews while it encodes; `state=cancelled` marks a withdrawn attempt (and earlier ones only, so a re-dispatched attempt inside the TTL is never cancelled by its own predecessor's marker). Not status: leases are ephemeral, `TranscodeJob.status` carries none of it |
...
**Since M7 (2026-09-24).** `catalogarr` gains a fifth role, `artwork`
(spec §C.6): `clustarr catalogarr --role artwork` runs the durable
`catalogarr-artwork-render` consumer alone, is not leader-elected and
scales by consumer; the chart and kustomize run it on the same catalogarr
Deployment/pod as the other roles. The metadata gateway (`--role metadata`)
gains the sole write of `original` artwork objects and `status.artwork`,
plus ratings enrichment (TMDB; MDBList and OMDb declared but not built,
ruling R5, §9); a new `overlayprofile` controller (`--role controller`)
computes each `OverlayProfile`'s hash and selection and publishes render
tasks. `ui` gains a read-only NATS connection (flags `--plex-provider`,
`--external-url`/`CLUSTARR_EXTERNAL_URL`) and now serves
`GET /art/{kind}/{uid}/{type}` (spec §B.8) and the Plex Metadata Provider
roots `/plex/movies` and `/plex/tv` (spec §D) alongside its existing pages
— still never writing status and never importing `pkg/k8s`.
...
### 6.4 squasharr (`app/squash/`, `clustarr squasharr --role controller`)

**As built by the transcode-worker-pools plan** (ADR-0009; supersedes the
per-TranscodeJob `batch/v1` Job this section described through Phase E; full
detail, including the NATS topology and the platform facts it rests on, is
`docs/superpowers/specs/2026-09-23-transcode-worker-design.md` §1-§18, cited
below by section). `--role worker` is gone: transcoding runs in a separate,
Kubernetes-credential-less binary, `cmd/squasharr-worker`, never inside the
`squasharr` process.

`transcodeprofile`: `Watches(&MediaFile{})` mapped to profiles whose selector matches labels (default when none), predicate = `status.probeHash` changed; compliance = hevc + main10 + yuv420p10le + AAC-only audio + container + `CLUSTARR_PROFILE=<name>@<hash>` tag; creates `TranscodeJob` (create-if-absent by deterministic name `<mediafile>-<hash[:8]>`). `transcodejob`'s phases are unchanged in name -- Pending → Planned (`pkg/transcode.Planner`; a Dolby Vision `reject` or a container change lands as `Skipped` with a reason) → Queued → Running → Succeeded/Failed/Skipped -- but Queued now means the job's task is published on the queue, not that a per-job Job was unsuspended, and nothing here mirrors Job conditions any more (design §8). **`Verifying` is legacy** (final fix round, kind finding D5): it stays in the enum, so an object the pre-pool controller wrote still validates, but nothing sets it under pools -- the worker verifies inside Running and reports one `finished` event -- and a reader that meets it treats it as Running.

**Dispatch.** In one pass (`MaxConcurrentReconciles` stays 1, so slot accounting and pool sizing can never disagree), admission publishes the highest-priority, oldest Planned jobs into free slots against `--slots cpu=2,nvidia=1,intel=1` and each profile's `maxConcurrent`, and sizes every pool from what it just dispatched. A pinned `Hardware` (`cpu`, `nvidia`, `intel`) never falls back. For the profile default, `hardware: auto`, the class is chosen nvidia, then intel, then cpu: a GPU class needs a GPU node of that class, a free slot counting this pass's own dispatches, no `fallbackReason` on the job, and its (profile, class) pool not marked unschedulable; a GPU pool with a pod stuck `PodScheduled=False,Unschedulable` for over 10 minutes is marked unschedulable for 30 minutes, and its Queued-but-unclaimed jobs are withdrawn and redispatched, which sends them to CPU (design §18.5). Dispatch plans for the chosen class and writes `status.plan` and `status.hardware` in the same write, and publishes `schema.TranscodeTask` (jobRef, the profile's render spec, source/output paths, root folder, `argsHash`, deadline) to `clustarr.work.transcode.task.<profileUID>.<class>.<jobUID>`, Msg-Id `<jobUID>/<status.attempts>` (design §5-§6).

**The pool.** One `batch/v1` Job per (TranscodeProfile, hardware class), `squasharr-pool-<profile>`, owned by the TranscodeProfile, running `squasharr-worker` pods from `--worker-image` (cpu, intel) or `--worker-image-cuda` (nvidia) -- the `transcoder`/`transcoder-cuda` images (design §10), never `media`/`media-cuda` -- with no ServiceAccount at all (`automountServiceAccountToken: false`). A GPU pool is **created** with `.spec.scheduling.schedulingConstraints.topology: [{key: <the class's GPU node label>}]` (default `nvidia.com/gpu.present`/`intel.feature.node.kubernetes.io/gpu`, overridable with `--gpu-node-label-nvidia`/`--gpu-node-label-intel`); that constraint and `spec.scheduling.schedulingPolicy.gang.minCount = parallelism` are each immutable once set and re-sent on every apply, and nothing else lives under `.spec.scheduling`. A pool with no dispatched work is `suspend: true` (a gang policy can never reach `parallelism: 0`); otherwise `parallelism = min(dispatched, profile.maxConcurrent, the class's remaining --slots)`, floored at 1. A mutable change (resources, nodeSelector, tolerations) drains the pool and re-templates it once idle; an immutable change (image, runtime class, the GPU constraint) drains, then deletes and recreates it. Each pool's durable pull consumer, `squasharr-transcode-<profileUID>-<class>`, is created by its workers' first `Pull` and kept while the TranscodeProfile with that UID exists, idle or not; once no profile has that UID, the five-minute sweep (below) deletes it, with its dead-letter watcher on `CLUSTARR_ADVISORIES` (`StreamAdmin.Subscriptions` lists a stream's durables, `DeleteSubscription` removes both; final-review M1, kind finding D1 -- until then nothing deleted either, so every deleted or recreated profile leaked them). It is **not** in `pkg/events.Default()` (design §5, §7). The pool's `podFailurePolicy` ignores a `DisruptionTarget` pod and the exits `WorkerExitDrained` (10) and 137 (an OOM-killed container), and fails the Job on `WorkerExitMisconfigured` (3): a pool never finishes, so its `backoffLimit` of 6 is spent over its whole life, and a Failed pool takes its healthy pods' encodes with it (final-review I1). A task that OOMs on every delivery is still bounded, by its consumer's MaxDeliver 16, then the dead letter, then Blocked. A Job's `podFailurePolicy` is immutable, so a pool created before a policy change refuses every apply; that refusal, like a refused gang `minCount`, reads as immutable drift -- an idle pool is recreated at once, a busy one drains first (`pool.IsRecreateOnly`).

**Finding a pool, and holding one.** A pool is found by the name its profile's UID and class derive (`pool.Name`), never by reading its labels back -- the profile label is only label-safe display, not identity (ruling R19) -- and by its controller owner reference, which must name this exact TranscodeProfile. Admission holds a `Planned` job, with a message saying why, for a pool that is: draining (its template has drifted and its pods may still be running the old one); `Failed` and inside its recreation backoff (1m, doubling on each failure that follows the last wait closely, capped at 30m, ruling R20); or whose name is taken by a Job owned by something else, which only an operator's edit of `ownerReferences` produces, since the name already hashes the profile's UID (ruling R22 -- squasharr neither adopts nor deletes a Job it does not own; deleting it by hand lets the pool be recreated). A held job's already-dispatched tasks stay queued; a drain lasts as long as the pool's slowest dispatched job (`app/squash/controller/transcodejob/pools.go`'s `holding`).

**The lease protocol and withdrawal.** A worker claims a task with a compare-and-swap create in `clustarr-transcode-leases` (bucket TTL 90s, server-expired, so worker clock skew cannot matter), renews it while it encodes, and stops itself at least 30s before the lease could expire. Immediately before the swap it re-asserts the lease once more with a revision-checked update rather than trusting the last periodic renewal, which can be stale by nearly the renewal interval (final-review I2c): a failure aborts without swapping, removes this attempt's own part file, and settles exactly as a renewal-time fence or cancel would -- otherwise a withdrawn attempt (it learns of cancellation only at its next renewal) or a second worker that already took the lease could still see its output swapped into the library. Withdrawing a job that stays (suspend, reroute, an orphan) writes a `cancelled` marker first, scoped to that attempt and earlier ones only, so a job withdrawn and re-dispatched inside the lease TTL is never cancelled by its own predecessor's marker; it then purges the still-queued task from the work stream by `clustarr.work.transcode.task.*.<class>.<jobUID>` -- the profile token is wildcarded (ruling R23), so withdrawal needs no TranscodeProfile and still works once the profile is gone. **Deleting** a job withdraws every attempt instead (final-review M6): its marker names attempt `MaxInt32` -- a job recreated under the name has a new UID, so a new lease key -- and it purges `clustarr.work.transcode.task.*.*.<jobUID>` whatever `status.hardware` says, because a dispatch whose `Queued` write was lost has published `attempts+1`, possibly under a class `status.hardware` never recorded, and that task would otherwise replace an attempt-scoped marker at its claim and run for a job that is gone. A five-minute backstop sweep purges any task subject left with no live TranscodeJob behind it at all (a delete that raced a NATS outage, or a lost `Queued` write); the stream is DiscardNew with no MaxAge, so an unswept leak would otherwise only grow toward its cap. The same sweep deletes the pool durables of every TranscodeProfile that no longer exists (above), comparing each durable against the names built from the existing profiles' UIDs and the pool classes, never parsing a UID back out, and never touching `squasharr-transcode-results` (design §8; `app/squash/controller/transcodejob/withdraw.go`).

**Orphans (final-review C1).** A TranscodeJob carries no owner reference to its profile, but its pools do, so deleting a profile -- or deleting and recreating it under the same name, which gives it a new UID and new pools -- leaves its `Queued` and `Running` jobs dispatched to pools that are gone, on a subject nothing pulls. A dispatched job is orphaned when its profile is gone or its `status.jobRef` is not `pool.Name` of the current profile for its `status.hardware` (every dispatch and adoption records exactly that name). Its reconcile withdraws it (the attempt-scoped withdrawal above) and then fails it `ProfileDeleted` -- `Failed=True`, **not** Blocked, so recreating the profile and deleting the job retries it -- when the profile is gone, or returns it to Planned (`JobCreated=False`, reason `Orphaned`) when the profile exists again, so its next dispatch is a new attempt to the current profile's pool. Admission does not wait for that reconcile: an orphan takes no slot of its class and counts toward no pool, so a recreated profile's pool never runs idle for a task it will never see (`app/squash/controller/transcodejob/controller.go`'s `recoverOrphan`, `dispatch.go`'s `orphaned`).

**Results and the one CAS status writer.** The worker publishes `transcode.StatusEvent.v1` (`claimed`, `progress` at most every 10s, `finished`, each carrying its `class`) to `clustarr.work.transcode.result.<jobUID>`, acking its task only once JetStream has durably stored `finished`. squasharr's static `squasharr-transcode-results` consumer (AckWait 30s, MaxDeliver 10, BackOff 5s/30s/2m, MaxAckPending 1) is the only other writer of `TranscodeJob.status` besides the reconciler, and both go through one function: read the object fresh through the uncached reader, change a copy of its status, apply it with the read `resourceVersion` as a precondition. A write that races the other path fails Conflict and is redone from a fresh read -- never silently rolled back. `claimed`/`progress` move a job to Running and set `workerPod`/`startedAt`/`progress`; an event whose `attempt` matches neither `status.attempts` nor an adoptable one (next), or that reaches a terminal job, is acked and changes nothing (design §18.1-§18.2).

**Adoption (ruling R16).** An event for `status.attempts+1` on a `Planned` job is proof a task was published and dispatch's own `Queued` write was lost: the consumer adopts it in the same CAS write, marking the job `Queued` for that attempt and applying the event itself, rather than dropping the worker's report and leaving a dispatched job with no recorded task behind it. A `claimed`/`progress` event for the job's current attempt whose `class` differs from `status.hardware` corrects `status.hardware` and `status.jobRef` in that write too -- the case where a re-dispatch to another class was absorbed as a duplicate of the attempt's first publish, and the task ran where the worker actually took it from (`app/squash/controller/transcodejob/results.go`). Adoption is also routine without any lost write: an idle pool pod claims within milliseconds of the publish, so its `claimed` can land before dispatch's own `Queued` write. When dispatch then finds the job already dispatched at its attempt and class, it records the re-plan it published (a GPU plan admission chose, or a cpu one after a fallback) -- `status.plan` and the Planned condition, nothing else -- instead of dropping it, so they, and every later history event's `encoder`, name the plan the task carries. The adopting write's own `queued` and `started` history events were published before that and name the earlier plan.

**The next-step table** (keyed on `finished.outcome`/`reason`, design §18.3): succeeded/skipped → Succeeded/Skipped; cancelled → nothing (withdrawal already acted); a GPU-only reason (`GPUUnavailable`, `GPUEncodeFailed`) on an `auto` job → `status.fallbackReason` set, back to Planned, the next dispatch is CPU; `Retriable`, or a GPU reason on a pinned-GPU job → Planned with `status.nextAttemptAt` at 1m/5m/15m/30m for up to 5 attempts, then blocked `RetriesExhausted`; `SourceChanged` → Failed, **not** blocked (the TranscodeProfile controller deletes and recreates a job whose `spec.sourceProbeHash` no longer matches its MediaFile's current probe hash); `InvalidSource`, `VerifyFailed`, `DeadlineExceeded`, or a dead-lettered task → blocked. **Blocked is not a new phase:** it is `phase: Failed` plus a `Blocked=True` condition (seven condition types now, `MaxItems=8`) carrying the reason and message; the job's fixed name stops the profile controller creating it again, so the supported retry is `kubectl delete transcodejob` (design §18.4).

**The worker and its two images.** `cmd/squasharr-worker` never exits 0: `WorkerExitRetriable=2` (NATS unreachable, ffmpeg missing -- retried), `WorkerExitMisconfigured=3` (bad or missing environment; the pool's `podFailurePolicy` fails the Job on it), `WorkerExitDrained=10` (SIGTERM mid-task; `podFailurePolicy` ignores it, so scaling a pool down never fails the Job); an OOM-killed worker (exit 137) is ignored too (final-review I1, above). `Serve`'s own pull -- the one call every idle worker makes against its pool's durable consumer, including its very first, at startup -- retries a transient error (a JetStream consumer-leader move, a 503 during a node restart) with backoff up to 30s, giving up and exiting only once the failure has persisted for five minutes with no successful pull in between (final-review I1, R29 fix 1/2), so a brief leader election does not spend the pool Job's `backoffLimit` and kill every other healthy encode sharing it. It probes (reuse MediaFile info + its own ffprobe for DOVI/frame side data), runs `ffmpeg -progress pipe:1 -stats_period 1 -nostats -loglevel error` with `pools=<cpu limit>`, writes `<stem>.part-<uid8>-<attempt>.mkv` -- unique per job and attempt (final-review I2a), so a withdrawn attempt's cleanup or an old- and new-hash job for the same file can never collide on, or delete, a different attempt's in-progress file -- verifies (`-count_packets` equality, ±1 frame video / ±100 ms audio, optional full decode / libvmaf), tags `CLUSTARR_PROFILE`, and atomically renames the output over the source path (source → recycle bin), or to a new name per R-11 (§4.5) -- all unchanged from Phase E (design §9) except that it reads its `TranscodeTask` off NATS instead of the apiserver, and holds no Kubernetes credentials at all: no ServiceAccount token, no RBAC. The `transcoder`/`transcoder-cuda` images (`images/Dockerfile.transcoder`, design §10) are the only ones that ever run an encode, and the only ones carrying the Intel QSV/VAAPI and NVIDIA runtimes; `media`'s ffmpeg is the same BtbN build and links libx265 too, but nothing in the project asks it to encode -- it only probes and, for captionarr's embedded provider, extracts. A TranscodeProfile skips a MediaFile whose Movie or Episode is gone (an ImportList `removeAndKeep`); the TranscodeJob controller emits Kubernetes Events and publishes job events (§5). Transcoding replaces the library hardlink only; a seeding copy in `/data/torrents` is untouched.

`TranscodeJob.status` has exactly one writer, `k8s.ManagerSquasharr`; the `squasharr-worker` ServiceAccount and its RBAC role are gone, and so is the KEDA transcode example (`config/keda`, §12 below).

**Upgrade note.** A TranscodeJob left `Queued` or `Running` by the pre-pool controller (the `batch/v1`-per-TranscodeJob design this section described through Phase E) carries a stale `squasharr-worker` `managedFields` entry from the field manager that no longer exists. Its `status.jobRef` names its own `batch/v1` Job, not a pool, so since the final fix round the orphan rule (above) withdraws it and returns it to Planned, and it is dispatched to its pool as a new attempt; until then nothing ever advanced it and it counted toward `maxConcurrent`. Deleting it is still the way to drop the stale entry; the TranscodeProfile controller recreates it under the new design. Pre-alpha, no deployed users, so no migration path was built (Task 10, parked deviation 6).

*Built since (gap fixes Z5; the NATS dependency below is superseded by the transcode-worker-pools plan).* **Telemetry.** The worker writes `schema.TranscodeProgress` into `clustarr-progress` under `transcode.<uid>` (§5), at most once a second and only when a new sample arrived, one Put in flight with a 2 s timeout, a failed Put dropped and logged once, so telemetry can never fail or slow an encode. `NATS_URL` is no longer optional the way Phase E's ruling R6 ("the worker needs no bus") had it: the transcode-worker-pools worker has no Kubernetes access at all, so every pool pod carries `NATS_URL` to receive its task in the first place, not only to report progress. `status.progress` remains what the UI reads. **CPU default.** A profile with no CPU limit gets a literal `CLUSTARR_CPU_LIMIT` on the pool pod -- the CPU request rounded up, else 8 -- and the planner uses the same number, so `status.plan`'s `pools=` is the worker's argv. **Stream cap.** `transcode.Plan` rejects, as its first check, a source with 64 or more audio or subtitle streams (`transcode.MaxStreamsPerKind`, the stored summary's cap), with one reason whichever path plans it, so the controller and the worker always decide alike. **Already transcoded.** The planner and the TranscodeProfile controller share one check, `worker.HasProfileTag`: a match in either `status.transcode.profileTag` or the probe's `status.mediaInfo.transcodeProfile` counts, so a kept source whose own probe tag differs is not planned again.
...
```
UMASK is applied by the binary itself, in the root command, for every subcommand (gap fix X14; the manifests set `UMASK=002`, an unparseable value is refused, and grabarr passes its own to the engine pods it builds). Node-local scratch: usenet engine `/scratch` (emptyDir or NVMe StorageClass ephemeral volume), transcode Jobs emptyDir (`profile.scratch`). *As built (2026-09-24, ADR-0014):* the usenet engine's working area is placed by `spec.usenet.scratch` -- a `path` on the shared data volume (`/data/usenet/incomplete`, so a transfer survives the pod and publishing to `spec.usenet.publishDir`, `/data/usenet/complete`, is one rename), an operator's `existingClaim`, a claim the controller makes from `storageClassName` or bound statically to `volumeName` with the `accessModes` asked (ReadWriteMany for NFS), or the emptyDir. The emptyDir is the default and is the wrong choice on a single-node cluster: it shares the node disk with etcd, and a 10 GB transfer plus its par2 verify starved etcd until the apiserver was killed by its liveness probe; and it discards a finished transfer with the pod. Never put anacrolix completion DBs/mmap on NFS (defaults avoid them). Other stores: etcd (CRs), JetStream file store PVC 20Gi R3, SQLite PVC `clustarr-index` 5Gi RWO (the SQLite-only shape since M7, 2026-09-24: unmounted, no PVC at all, when `--index-dsn` selects Postgres instead, ADR-0010), Secrets (provider keys, indexer credentials/sessions, Trakt tokens).

**Since M7.** Postgres is now a selectable release-index engine (`--index-dsn`), provisioned by an optional CloudNativePG `Cluster` the chart (`postgres.enabled`) and kustomize (`config/postgres`) can render — no Postgres is required, and the SQLite PVC above is the unchanged default (ADR-0010). A JetStream object store bucket, `clustarr-artwork` (5Gi, file storage, ADR-0011), holds provider-fetched and custom artwork; `ForSingleNode` maps every object store, not only KV buckets and streams, onto file storage, because mapping the artwork bucket to memory crash-looped every controller on a single-node kind cluster trying to reserve 5Gi of memory storage (CLAUDE.md gotchas). Redis remains unused.

## 12. Scheduling / scaling

Controllers 1 replica each, leader-elected (catalogarr workers scale horizontally; `catalogarr-metadata` and `indexarr` pinned to 1). Torrent engines: StatefulSet ordinals = `DownloadClient.spec.replicas`, ≤500 torrents/pod, `GOMEMLIMIT` = 80 % of limit (grabarr computes it from `DownloadClient.spec.resources.limits.memory` when it builds the engine workload, gap fix X14; the chart's `clustarr.gomemlimit` helper and `config/manager` literals do the same for every Deployment, gap fix X12a; none without a limit). Transcode (ADR-0009, §6.4): one pool `batch/v1` Job per (TranscodeProfile, hardware class), suspended to zero when it has no dispatched work and otherwise gang-sized to `parallelism` (`spec.scheduling.schedulingPolicy.gang.minCount`, on clusters with `WorkloadWithJob`), admitted entirely by squasharr's own dispatcher against slot budgets (`--slots cpu=2,nvidia=1,intel=1`) and each profile's `maxConcurrent` -- no KEDA leg at all, queue depth never sizes a pool. Pod sizes and NVIDIA time-slicing are unchanged: 8 CPU/4Gi (1080p) and 8 CPU/8Gi (2160p), replicas ≤8 (default 4). Subtitle workers fixed 2 replicas. **KEDA is opt-in** (`keda.enabled=false` default; chart dependency 2.20.2, the newest release on 2026-09-23 -- tested upstream against Kubernetes 1.33-1.35, and no KEDA release has yet been tested against 1.37, gap fix X12a): when enabled, ScaledObjects on `captionarr-worker` and catalogarr search workers use the `prometheus` trigger on `jetstream_consumer_num_pending + jetstream_consumer_num_ack_pending` from `prometheus-nats-exporter` until kedacore/keda#8166 ships, `cooldownPeriod ≥ AckWait`, `terminationGracePeriodSeconds` = AckWait, workers drain on SIGTERM. Homelab: NATS 3×(100m/256Mi req, 1 CPU/1Gi lim), fileStore 20Gi; total messaging overhead < 1 GiB.
...
images/{Dockerfile.controller,Dockerfile.media,Dockerfile.transcoder}   (Dockerfile.media-cuda is gone; Dockerfile.transcoder builds both the transcoder and transcoder-cuda targets, ADR-0009)
...
- **M7 Index/artwork/ratings/Plex (done, 2026-09-24):** `pkg/relindex` gains a selectable Postgres engine (`--index-dsn`, CloudNativePG, ADR-0010); a JetStream object store (`clustarr-artwork`) holds provider and custom artwork, served by `ui` at `/art` (ADR-0011); `status.metadata.ratings` from TMDB for movies only (series carry none until TMDB TV ratings or MDBList/OMDb land), with MDBList and OMDb declared but not built (ruling R5, no API keys); `OverlayProfile` and the `catalogarr --role artwork` renderer composite Kometa-style rating badges onto posters; `ui`'s `/plex/movies` and `/plex/tv` serve Plex's Custom Metadata Provider protocol (ADR-0012). Design: `docs/superpowers/specs/2026-09-24-index-artwork-ratings-plex-design.md`.
...
- **ADR-0003 Release index = SQLite FTS5 (modernc) on RWO PVC, single writer, Store interface.** Rebuildable cache; Postgres FTS is the multi-replica path. **Superseded by ADR-0010**, 2026-09-24.
...
- **ADR-0005 Transcodes are batch/v1 Jobs gated by suspend + slot budgets**, not queue messages; chunking = Indexed Jobs later. **Superseded by ADR-0009** (below), 2026-09-24.
- **ADR-0006 Storage = one RWX volume at `/data`, TRaSH layout, hardlink-else-copy / atomic move; no external object store for media.** S3-compatible storage stays rejected for the library (no hardlinks, no atomic rename, ffmpeg wants a seekable file); JetStream's own object store is in use since M7, for artwork only (ADR-0011).
...
- **ADR-0009 Transcodes run on one long-lived worker-pool Job per (TranscodeProfile, hardware class)**, fed by a JetStream work queue instead of one `batch/v1` Job per TranscodeJob (supersedes ADR-0005). Kubernetes 1.37's mutable scheduling directives for a suspended Job and gang admission via `spec.scheduling.schedulingPolicy.gang.minCount` let squasharr reshape and gang-size a pool without a KEDA operator; the worker (`cmd/squasharr-worker`, the `transcoder`/`transcoder-cuda` images) holds no Kubernetes credentials at all and talks only to NATS, fenced by a server-expired lease rather than the ack window ADR-0005 rejected a queue for. Revisit: `WorkloadWithJob` changing shape or graduating; partial (not only to-zero) pool scale-down; chunked transcoding, which needs Indexed pools.
- **ADR-0010 The release index engine is selectable: SQLite FTS5 by default, Postgres FTS via CloudNativePG for multi-replica indexarr** (supersedes ADR-0003). `pkg/relindex` gains `OpenPostgres` behind the same `Store` contract, held to SQLite by one shared suite (`storetest.Run`); `--index-dsn` (`CLUSTARR_INDEX_DSN`) picks the engine at startup, a pivot from empty rather than a migration, since the index is a rebuildable cache. A DSN turns on leader election for every indexarr controller and the retention sweep, because more than one replica may now run; the chart and kustomize ship an optional CloudNativePG `Cluster` (`postgres.enabled`/`config/postgres`), and SQLite on its RWO PVC stays the default. Revisit: the Postgres query plan once real corpus sizes are observed; replicas pacing trackers per replica rather than sharing limiter state (an M7 carried item); removing SQLite as the default, only for a reason a single-replica homelab would notice.
- **ADR-0011 Artwork lives in a JetStream object store, one bucket (`clustarr-artwork`), two writers split by variant.** Supersedes amendment 1 §A3.4's `/data` disk-cache sketch, which was never built past the sentence (Phase G shipped hotlinking provider URLs instead); leaves ADR-0006's RWX `/data` volume for the primary media library untouched — artwork is small, wholly derived and disposable. The metadata gateway is the sole writer of `original` objects and `status.artwork`; the overlay renderer (`catalogarr --role artwork`) is the sole writer of `overlay` objects and `status.overlay`; `ui` gains a read-only bus connection to serve `/art`, never a write path. Revisit: a lower-trust caller (e.g. `ui/actions`) gaining a write path to `spec.artwork`, which needs a private-address deny-list first; the bucket approaching its 5 GiB cap.
- **ADR-0012 The Plex Metadata Provider is served read-only by the `ui` service, unauthenticated per protocol, and must not be publicly exposed.** Mounted on `ui` rather than a new service or `catalogarr`, because `ui` is the one process structurally incapable of writing (`ui/guard_test.go`); gated by `--plex-provider` and `--external-url`/`CLUSTARR_EXTERNAL_URL`, the base every image URL is built on. Revisit: Plex shipping authentication for custom providers; `ui` gaining any write path of its own; Phase H's real-PMS run settling 0- versus 1-based paging (design §D.2).
