...
WorkQueue retention is immutable: `natsbus.Ensure` refuses to start if an existing stream's retention differs (operator must migrate). grabarr and squasharr have no work stream. The work streams are **DiscardOld**, not the DiscardNew this table first asked for: nats-server refuses DiscardNew beside message schedules ("message scheduling cannot use discard new"), so the streams are sized with headroom that makes the limit an alarm, and publishers still handle `ErrQueueFull`. This table matches `pkg/events.Default()` (gap-fix X13 brought it up to date: seven streams, fifteen consumers, eleven buckets).
...
| `clustarr.evt.download.download.queued\|started\|completed\|seedGoalMet\|imported\|failed\|blocklisted\|removed.<uid>` | `download.DownloadEvent.v1` | grabarr Download controller on phase edges (gap fix X9; `seedGoalMet` has no producer yet) → history |
...
*As built:* grabarr publishes `evt.download.download.failed` on the edge, and the item's reconciler stops naming the Download in `activeDownloadRef` because a Failed Download is terminal (R-5); nothing deletes the lease, which the next grab reclaims (§8.2). Two clauses are **not built (open)**: nothing sets the blocklist label on a Failed Download (only an operator or a test does; grabarr honours the label once set), and nothing publishes a `redownload` search (`SearchReasonRedownload` has no producer) -- the item reads Wanted again and the wanted sweep searches it on its next pass. Direct-URL grabs count into the grab window from indexarr's own Download watch (§6.2).
