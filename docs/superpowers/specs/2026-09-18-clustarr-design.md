...
**The lease protocol and withdrawal.** A worker claims a task with a compare-and-swap create in `clustarr-transcode-leases` (bucket TTL 90s, server-expired, so worker clock skew cannot matter), renews it while it encodes, and stops itself at least 30s before the lease could expire. Withdrawing a job (suspend, delete, reroute) writes a `cancelled` marker first, scoped to that attempt and earlier ones only, so a job withdrawn and re-dispatched inside the lease TTL is never cancelled by its own predecessor's marker; it then purges the still-queued task from the work stream by `clustarr.work.transcode.task.*.<class>.<jobUID>` -- the profile token is wildcarded (ruling R23), so withdrawal needs no TranscodeProfile and still works once the profile is gone. A five-minute backstop sweep purges any task subject left with no live TranscodeJob behind it at all (a delete that raced a NATS outage, or a lost `Queued` write); the stream is DiscardNew with no MaxAge, so an unswept leak would otherwise only grow toward its cap (design §8; `app/squash/controller/transcodejob/withdraw.go`).
...
**Upgrade note.** A TranscodeJob left `Queued` or `Running` by the pre-pool controller (the `batch/v1`-per-TranscodeJob design this section described through Phase E) is never advanced after upgrading to worker pools: nothing dispatches its task, it still counts toward `maxConcurrent`, and it carries a stale `squasharr-worker` `managedFields` entry from the field manager that no longer exists. The fix is to delete it; the TranscodeProfile controller recreates it under the new design. Pre-alpha, no deployed users, so no migration path was built (Task 10, parked deviation 6).
