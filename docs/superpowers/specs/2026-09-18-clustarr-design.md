...
```
An indexer outcome the RPC returned without a name is kept as `catalogarr/unnamed-indexer-<n>`, and a truncated reply adds a `catalogarr/truncated` marker that keeps its slot at the cap (gap fix X4b). An interactive grab maps a release to `spec.source` through the same resolver as the automatic grab (§8.2).
...
WorkQueue retention is immutable: `natsbus.Ensure` refuses to start if an existing stream's retention differs (operator must migrate). grabarr and squasharr have no work stream. The work streams are **DiscardOld**, not the DiscardNew this table first asked for: nats-server refuses DiscardNew beside message schedules ("message scheduling cannot use discard new"), so the streams are sized with headroom that makes the limit an alarm, and publishers still handle `ErrQueueFull`. This table matches `pkg/events.Default()` (gap-fix X13 brought it up to date, and gap fix Y3 added `catalogarr-redownload`: seven streams, sixteen consumers, eleven buckets).
...
| `clustarr.evt.catalog.mediafile.imported\|replaced\|deleted.<uid>` | `catalog.MediaFileEvent.v1` (droppedPath, importedPath, reason manual\|missingFromDisk\|upgrade) | catalogarr (Movie and Episode reconcilers as `fileRef` changes; `<uid>` is the item's, gap fix X5a) → history |
...
| `clustarr.work.catalogarr.import.normal.<download-uid>` | `catalog.ImportTask.v1` {downloadRef} | **unused**: amendment §A1 moved import to `clustarr.work.importarr.fileimport.<download-uid>` below; this subject, `FilterCatalogImport` and the `catalogarr-import` consumer are pre-amendment leftovers with no subscriber (open item) |
...
| `clustarr.work.indexarr.definitions.normal.sync` | `index.DefinitionsSync.v1` | **unused**: no worker was built; with nothing vendored (R-13) definitions arrive through the startup bundle loader (§4.3) (open item) |
...
| `catalogarr-import` (declared, **no subscriber**: import is `importarr-fileimport`; open item) | WORK_CATALOGARR | `…import.>` | 300s | 5 | 30s,2m,10m | 4 | 60s |
...
| `indexarr-definitions` (declared, no subscriber; open item) | WORK_INDEXARR | `…definitions.>` | 300s | 3 | 5m,30m | 1 | – |
...
DLQ path: handler returns `events.Discard` → copy to `clustarr.dlq.*` → `TermWithReason`; a handler still failing on its final delivery (attempt = MaxDeliver) is copied and terminated the same way, in process (`events.Settle`, shared by natsbus and membus). **A hung handler (gap fix Y1):** a message whose last delivery expires on AckWait never returns to `Settle`, so natsbus watches the core-NATS advisory `$JS.EVENT.ADVISORY.CONSUMER.MAX_DELIVERIES.<stream>.<durable>`. Each `Subscribe` subscribes before it starts consuming, in the queue group `clustarr-dlq-watch-<durable>`, so one replica handles each advisory. That replica reads the message back by stream sequence and publishes the DLQ copy with the same `Clustarr-DLQ-*` headers and the same Msg-Id, `dlq:<durable>:<original id>`, as the in-process path, so a copy made twice is one copy. The reason is `events.AckWaitExhaustedReason` ("max deliveries exceeded (n): acknowledgement timed out"). On a WorkQueue stream it then deletes the original, as a Term would; JetStream keeps a WorkQueue message after the advisory indefinitely. EVENTS and RELEASES keep theirs. membus gets the same behaviour from an ack-deadline sweep that dead-letters a lapsed final delivery even while every handler slot is held. The watcher runs in every process that owns the consumer, not in a leader: the DLQ path stays in process, and a watcher in catalogarr's history role would make every service's DLQ depend on that role being deployed. **Open:** the advisory is core NATS, so it is lost if no replica of that consumer is subscribed when it fires; natsbus runs one handler at a time per subscription, so a handler hung forever on an earlier delivery stalls that replica's consumer until a restart or another replica takes the message; a hung handler that errors more than 10 minutes after the advisory (`CLUSTARR_DLQ`'s duplicate window) adds a second copy, which the projector's idempotent annotation absorbs; membus times a redelivery on AckWait, not BackOff. **Projection (Phase G ruling R1, gap fixes X1/X5a):** the DLQ projector does not write status. It applies the annotation `clustarr.io/dead-lettered: <subject>@<RFC3339>` (and, when it can read the stream, `clustarr.io/dead-letter-seq: <seq>`) under `clustarr-dlq-projector` to the one object the payload resolves to -- sixteen kinds, listed in `pkg/k8s/deadletter.go` -- and emits a Warning Event there; a dead letter that names only a namespace gets its Event in that namespace. Each owning controller folds the annotation into its own `DeadLettered` condition (`k8s.MarkDeadLettered`, present only while the annotation is). `kubectl annotate <cr> clustarr.io/replay=<dlq-seq>` is served by `catalogarr/history.Replayer` (history role, NATS only): it reads that sequence from `CLUSTARR_DLQ`, refuses it unless it resolves to this exact object, republishes it to its original subject without the DLQ headers under Msg-Id `replay:<seq>:<uid>`, and removes the replay annotation -- and the dead-lettered pair when the sequence matches -- with a value-tested JSON patch; a refusal is a `ReplayRefused` Warning.
...
| `clustarr-search-cache` | `<indexer-uid>.<sha256(request)>` | 35m | Jackett-style raw-result cache (declared; nothing reads or writes it yet) |
| `clustarr-progress` | `scan.<scan-uid>` (rescan checkpoint), `importlist.<list-uid>` (sync result); `download.<uid>`, `transcode.<uid>` reserved | 10m | importarr's checkpoints, read by its controllers; no engine or worker writes 1 Hz telemetry here yet |
...
indexarr/    run.go  controller/{indexer,indexerdefinition,indexerproxy}/  search/ (micro rpc, fanout, dedup, download)  rss/  releaseindex/ (Store, sqlite/)  facade/  health/  defs/ (embedded v11 corpus + schema.json; as built: bundle/ + bundle/embedded/definitions.zip, packed by hack/pack-cardigann; schema.json in pkg/cardigann; proxy/ for IndexerProxy routing)
