...
- [ ] Follow-up (ruling R5): **MDBList done 2026-09-24** (fixtures recorded, `pkg/metadata/clients/mdblist`, optional `apiKeySecondary`); OMDb remains: record MDBList and OMDb fixtures and build the two clients once API keys are available (`test/data/metadata/mdblist/`, `test/data/metadata/omdb/`, `docs/research/ratings-providers.md`); until then `MetadataProviderType: mdblist|omdb` CRs report `Ready=False, InvalidSpec`.
...
- [ ] Final review (Imp2), fix two: **MDBList done 2026-09-24** -- it declares its sources for series and `TestRatingsKeysASeriesByTVDBWhenItHasNoTMDBID` rates one through the recorded show; OMDb remains. MDBList (`/tmdb/show/{id}`) and OMDb, once built under the R5 follow-up above, declare their sources for series as well as movies (spec §C.2's table), and a test proves a series is rated through them.
...
- [x] **Done 2026-09-24:** the badge is now Plex's episode-count box, measured (spec §C.5's second as-built note), with `scorePercent` of the box height. Was: `pkg/overlay`'s default geometry draws an unreadable score (found on kind-cluster-plex, 2026-09-24): `OverlayGeometry.ScorePercent` is documented, in the CRD and spec §C.4, as a percentage "of the badge's box height", but `drawBadge` applies it to the height left under the logo. With the defaults on a 500px poster that is a 70px square box less 10px padding three times and a 30px logo -- 10px -- so the cap height is 4.5px and the 8px font floor draws "75" about 7px tall. A square box cannot hold a 45%-of-box score under a 60%-wide logo with 2% padding, so the fix is a layout decision (a taller box, a smaller logo or inner padding, or the logo beside the score), not a one-line change. The cluster's `metascore` profile works around it with `geometry: {paddingPercent: 1, logoPercent: 45, scorePercent: 72}`.
