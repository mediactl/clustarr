...
- [ ] **`release.ClassifyKind` misreads brace-form ids, and `extractIDs` does not strip them — a release can end up with no parsed fields at all.** `Parse` strips ids *first* and classifies the stripped title (`pkg/release/parse.go:44-49`), but `extractIDs` does not recognise the `{tvdbid-121361}` brace form, so the id survives into the classified string. `ClassifyKind` then matches the brace against its audiobook narrator token, `Parse` refuses with "does not match any book title pattern", and the release ships with **no parsed fields whatsoever** — failing closed, so nothing matches rather than the wrong thing matching. Verified: `"Some Show S01E01 {tvdbid-121361}"` fails identically whether the kind is pinned or auto-classified, and `"{imdbid-tt0133093} The Matrix..."` leaves `"{imdbid-}"` behind in the title. **Note the earlier version of this entry blamed the wrong thing.** D1-7's classify-once workaround was reported as making this case worse; a review falsified that — the workaround is never worse and is strictly *better* for the leading-bracket shape (`"[tmdbid-603] Show - 12 [1080p].mkv"` parses under classify-once and fails under auto). So adding a `Kind` field to `ParsedRelease` would **not** fix the named reproducer. The two real fixes are teaching `extractIDs` the brace form and tightening `ClassifyKind`'s narrator token; the `Kind` field is a separate, smaller nicety. `pkg/release` change, no D1 task owns it.
