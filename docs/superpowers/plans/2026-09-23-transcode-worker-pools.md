...
**Goal:** Transcodes run in a separate `squasharr-worker` binary and image, with no Kubernetes
credentials. Its pods live in one long-lived Job per (TranscodeProfile, hardware class). squasharr
feeds those pods over NATS and sizes them, suspending a pool to zero when idle and reshaping it
while suspended. Where the cluster enables `WorkloadWithJob`, squasharr gang-admits each pool by
setting `gang.minCount` to the pool's `parallelism`.

**Architecture:** squasharr's TranscodeJob controller keeps planning and admission.
- **Dispatch.** It publishes each admitted job as a task on the `CLUSTARR_WORK_SQUASHARR`
  work queue.
- **Pools.** It renders and applies one pool Job per (profile, class), under the field manager
  `squasharr-pool`.
- **Status.** It is the only writer of `TranscodeJob.status`. It reads status back from three
  KV buckets: the lease, the result and the progress.
- **Worker.** The worker pulls one task at a time, claims it with a lease the server expires,
  renews that lease with `InProgress`, runs the existing encode, verify and swap sequence, then
  writes a result and settles the message.
...
**Spec:** `docs/superpowers/specs/2026-09-23-transcode-worker-design.md`. Read §17 (errata)
first: where it disagrees with §1-§16, §17 wins. ADR: `docs/adr/0009-transcode-worker-pools-over-jetstream.md`.

## Global Constraints

- Every new Go file starts with the GPL-3.0 header from `hack/boilerplate.go.txt`.
- **Status writes:**
  - All status writes go through `pkg/k8s.PatchStatus` (`squasharr/status.Patch`).
  - `.Status().Update()` and `.Status().Patch()` are banned by forbidigo.
  - After Task 10, `TranscodeJob.status` has exactly one writer: `k8s.ManagerSquasharr`.
- **Server-side apply:**
...
- **Pool applies:**
  - Every pool apply sends `spec.scheduling.schedulingPolicy.gang.minCount = parallelism`
    (≥ 1) and nothing else under `.spec.scheduling`.
...
1. **JSON drops a `false` policy pointer.** A task that round-trips JSON must keep
   ...
   zero value means "true", the same class as the typed-client gotcha.
   Test: `TestTaskJSONKeepsFalsePolicyPointers` (Task 3).
2. **Defaulted pod fields read as drift forever.** The apiserver copies `limits` into missing
   `requests`, so a naive comparison sees drift on every pass and the pool never resumes.
   Drift is judged against the template squasharr last applied, recorded in an annotation.
   Test: `TestClassifyIgnoresApiserverDefaulting` (Task 8).
3. **A re-dispatched job cancelled by its own stale marker.** A job withdrawn and re-dispatched
   inside the 90s lease TTL must not be dropped by the `cancelled` marker its first attempt left.
...
4. **SIGTERM right after a finished encode.** SIGTERM arriving after `Process` succeeded but
   before settlement must still write the result and ack, not nak and redo the work.
   Test: `TestServeFinishesASucceededTaskDespiteDrain` (Task 6).
5. **A profile edited while its pool runs.** No apply may be rejected: the running Job keeps
   its applied template, admission holds that pool's new work, and the reshape lands after
   suspend. Test: `TestProfileEditWhileRunningNeverRejectsAnApply` (Task 9).
...
Tasks 10 and 11 must land back to back. After Task 10 no pool exists yet, so dispatched tasks
wait in the queue. That is fine inside an unreleased branch, and nothing is pushed between them.

---

### Task 1: Transcode stream, subjects, consumer and buckets in `pkg/events`

**Files:**
- Modify: `pkg/events/subjects.go`: the stream, filter and bucket constants, plus the builders below.
- Modify: `pkg/events/topology.go`: the stream in `defaultStreams()`, both buckets in
  `defaultBuckets()`, and `TranscodeTaskConsumer`.
...
- Produces (every later task uses these):
  ...
  const BucketTranscodeResults = "clustarr-transcode-results"
  ...
  func TranscodeResultKey(jobUID string) string
...
func TestTranscodeTopology(t *testing.T) {
	...
	// A profile named with dots must not change the subject shape: UIDs only.
	subj := WorkTranscodeTaskSubject("6f1c-uid", "nvidia", "a1b2-uid")
	assert.Len(t, strings.Split(subj, "."), 7)
	...
	assert.Equal(t, StreamWorkSquasharr, c.Stream)
	assert.True(t, matches(c.Filters[0], subj))
	...
	assert.Equal(t, 8, c.MaxDeliver)

	var leases, results *BucketSpec
	for i := range top.Buckets {
		switch top.Buckets[i].Name {
		case BucketTranscodeLeases:
			leases = &top.Buckets[i]
		case BucketTranscodeResults:
			results = &top.Buckets[i]
		}
	}
	...
	assert.Positive(t, results.TTL)

	for _, k := range []string{TranscodeLeaseKey("a1b2-uid"), TranscodeResultKey("a1b2-uid")} {
		assert.True(t, ValidKVKey(k), k)
	}
...
Add `StreamWorkSquasharr = "CLUSTARR_WORK_SQUASHARR"` to the Stream const block (line 28),
`FilterWorkSquasharr = "clustarr.work.transcode.>"` to the Filter block (line 83), and both
`Bucket*` names to the Bucket block (line 120). Then append:
...
// TranscodeResultKey is a TranscodeJob's result in clustarr-transcode-results.
func TranscodeResultKey(jobUID string) string { return "result." + KVKeyToken(jobUID) }

// MsgIDForTranscodeTask carries the dispatch count, so a job withdrawn and
// dispatched again inside the duplicate window is not absorbed as a duplicate.
func MsgIDForTranscodeTask(jobUID string, attempt int32) string {
...
- [ ] **Step 4: Add the stream, buckets and consumer to `pkg/events/topology.go`**
...
```go
		{
			...
			Description: "Transcode tasks squasharr has admitted, one per dispatch.",
...
```go
		b(BucketTranscodeLeases, TranscodeLeaseTTL,
			"Transcode task leases: created by the claiming worker, renewed with Update, expired by the server."),
		b(BucketTranscodeResults, 7*24*time.Hour,
			"Transcode results by job UID; squasharr deletes each once it is recorded."),
```
...
// itself while it renews its lease (spec §17.3).
func TranscodeTaskConsumer(profileUID, class string) ConsumerSpec {
	return ConsumerSpec{
...
Run: `go test ./pkg/events/...`
Expected: PASS. A `ForSingleNode` test that pins exact per-stream byte sizes may now fail,
because one more stream shares the 64 MiB single-node total. If so, update only its expected
numbers to what `ForSingleNode` computes. The rules it tests (total ≤ 64 MiB, each stream
≥ `singleNodeMinStreamBytes`) do not change.

- [ ] **Step 6: Commit**

```bash
git add pkg/events/subjects.go pkg/events/topology.go pkg/events/transcode_topology_test.go
git commit -m 'feat(events): transcode task stream, per-pool consumer, lease and result buckets' -- pkg/events/subjects.go pkg/events/topology.go pkg/events/transcode_topology_test.go
```
(Include any `ForSingleNode` test file you updated in both commands.)
...
### Task 3: Task, result and lease types; `worker.BuildTask`
...
- Modify: `pkg/events/schema/transcode.go`: add `TranscodeProgress.BitrateKbps`.
- Modify: `squasharr/worker/telemetry.go`: fill in `BitrateKbps`.

**Interfaces:**
- Consumes: `events.TranscodeLeaseKey` (Task 1, used only in docs).
- Produces:
  ```go
  // package task
  ...
  type Reason string    // ReasonInvalidSource, ReasonVerifyFailed, ReasonDeadlineExceeded, ReasonRetriesExhausted, ReasonRetriable, ReasonCancelled
  type Result struct { Job schema.Ref; Attempt int32; Delivery uint64; Pod string; Final bool; Outcome Outcome; Reason Reason;
  	Message string; Result *transcodev1alpha1.Result; StderrTail string; StartedAt, FinishedAt time.Time }
...
func TestSchemasAreVersioned(t *testing.T) {
	...
	assert.Equal(t, "transcode.Result.v1", task.Result{}.Schema())
	assert.Equal(t, "transcode.Lease.v1", task.Lease{}.Schema())
	b, err := json.Marshal(task.Lease{State: task.LeaseCancelled, Attempt: 3})
	require.NoError(t, err)
	assert.Contains(t, string(b), `"state":"cancelled"`)
}
```

`squasharr/worker/buildtask_test.go`. This is a pure table test with in-memory objects:
...
// pools (spec §6, errata §17.5): the Task squasharr publishes, the Result a
// worker writes to clustarr-transcode-results, and the Lease a worker holds
// in clustarr-transcode-leases. It imports no Kubernetes client.
package task
...
// Reason qualifies a failed or cancelled Outcome.
type Reason string
...
// Result is what a worker writes for one delivery of one attempt. Final
// results end the TranscodeJob; the rest only report a failed try.
type Result struct {
	Job        schema.Ref                `json:"job"`
	Attempt    int32                     `json:"attempt"`
...
// Schema implements schema.Payload.
func (Result) Schema() string { return "transcode.Result.v1" }
...
// BuildTask renders one dispatch of tj: everything the worker used to read
// from the apiserver, resolved by the one function the controller and the
// worker's tests share (spec §6, errata §17.5).
func BuildTask(tj *transcodev1alpha1.TranscodeJob, tp *transcodev1alpha1.TranscodeProfile,
	...
	t := task.Task{
		...
		Profile: task.Profile{
			Name: tp.Name, Hash: tp.Status.Hash, Spec: *tp.Spec.DeepCopy(), Hardware: tj.Spec.Hardware,
		},
		SourcePath:      source,
...
- [ ] **Step 5: Carry bitrate in telemetry**

In `pkg/events/schema/transcode.go`, add this field to `TranscodeProgress` after `OutputBytes`:

```go
	// BitrateKbps is ffmpeg's current output bitrate.
	BitrateKbps int32 `json:"bitrateKbps,omitempty"`
```

In `squasharr/worker/telemetry.go`, set `BitrateKbps: p.BitrateKbps` wherever it builds
`schema.TranscodeProgress` from a `transcode.Progress`. Extend `telemetry_test.go`'s existing
assertion on the written sample with `assert.Equal(t, sample.BitrateKbps, got.BitrateKbps)`.

- [ ] **Step 6: Run the tests**

Run: `go test ./squasharr/task/ ./squasharr/worker/ ./pkg/events/schema/`
Expected: PASS. Envtest and ffmpeg suites skip without their assets; that is fine for this step.

- [ ] **Step 7: Commit**

```bash
git add squasharr/task squasharr/worker/buildtask.go squasharr/worker/buildtask_test.go squasharr/worker/profile.go squasharr/worker/telemetry.go squasharr/worker/telemetry_test.go pkg/events/schema/transcode.go
git commit -m 'feat(squasharr): task, result and lease types; BuildTask resolves what the worker used to read' -- squasharr/task squasharr/worker/buildtask.go squasharr/worker/buildtask_test.go squasharr/worker/profile.go squasharr/worker/telemetry.go squasharr/worker/telemetry_test.go pkg/events/schema/transcode.go
```

---

### Task 4: API: `status.workerPod` and the changed meanings

**Files:**
- Modify: `api/transcode/v1alpha1/transcodejob_types.go`: the `TranscodeJobStatus` docs, the new
  field and a print column.
- Modify: `api/transcode/v1alpha1/transcodeprofile_types.go`: docs for `ActiveDeadline` and
  `TTLSecondsAfterFinished`.
- Generated: `api/transcode/v1alpha1/zz_generated.deepcopy.go`,
  `api/applyconfiguration/transcode/v1alpha1/transcodejobstatus.go`,
  `config/crd/bases/transcode.clustarr.io_transcodejobs.yaml`,
  `config/crd/bases/transcode.clustarr.io_transcodeprofiles.yaml`.
...
```go
	// WorkerPod is the pool pod holding this job's task lease, so
	// `kubectl logs` can find it. Empty when no worker holds the task.
	// +optional
...
- The `TranscodeJobStatus` type comment at line 249, which says the worker (squasharr-worker)
  owns progress and result: replace that sentence with "squasharr writes every field; the worker
  reports over NATS."

Add a print column to the `TranscodeJob` type's markers (lines 314-320):

```go
// +kubebuilder:printcolumn:name="Worker",type=string,JSONPath=".status.workerPod",priority=1
```

In `transcodeprofile_types.go`:
- `ActiveDeadline`: "ActiveDeadline is each task's deadline, enforced by the worker; a task past it fails as DeadlineExceeded."
- `TTLSecondsAfterFinished`: "Deprecated: ignored. Transcode pools never finish; this is removed at the next API version."

- [ ] **Step 2: Regenerate**

Run: `make generate manifests`
Expected: the four generated files change; `git status --short api config/crd` lists them.

- [ ] **Step 3: Verify the CRD guards**

Run: `go test ./pkg/crdcheck/... ./api/...`
Expected: PASS (with `KUBEBUILDER_ASSETS` exported). `TestNoCRDDefaultIsUnreachableFromGo` and
`TestEveryStatusListIsCapped` still hold; the new field is a capped scalar.

- [ ] **Step 4: Commit**

```bash
git add api/transcode config/crd api/applyconfiguration
git commit -m 'feat(api): TranscodeJob status.workerPod; jobRef, attempts and deadlines say what pools make them' -- api/transcode config/crd api/applyconfiguration
```
...
### Task 6: The worker loop: pull, lease, renew, settle

**Files:**
- Create: `squasharr/worker/serve.go`, `squasharr/worker/lease.go`.
...
**Interfaces:**
- Consumes:
  - `events.PullSubscriber`, `events.TranscodeTaskConsumer`, `events.TranscodeLeaseKey`,
    `events.TranscodeResultKey` (Tasks 1-2)
  - `task.*` (Task 3), `Process` and `Outcome` (Task 5)
- Produces:
  ```go
  type ServeOptions struct {
  	Options                          // handed to Process for every task
  	ProfileUID, Class, Node string
  	Leases, Results events.KV
...
default single-node topology, and a stub `Process` the test controls:
...
type harness struct {
	...
	results events.KV
	calls   atomic.Int32
	release chan Outcome   // what the stub Process returns; unbuffered, so a send means it was taken
	started chan struct{}
	ended   chan struct{} // the stub returned
	done    chan error
}
...
func newHarness(t *testing.T) *harness {
	...
	h := &harness{t: t, clock: clockwork.NewFakeClock(), release: make(chan Outcome),
		started: make(chan struct{}, 8), ended: make(chan struct{}, 8), done: make(chan error, 1)}
	h.ctx, h.cancel = context.WithCancel(context.Background())
	t.Cleanup(h.cancel)
	h.bus = membus.New(h.clock)
	require.NoError(t, h.bus.Ensure(h.ctx, events.Default().ForSingleNode()))
	h.leases, h.results = h.bus.KV(events.BucketTranscodeLeases), h.bus.KV(events.BucketTranscodeResults)
	return h
}
...
func (h *harness) serve() {
	go func() {
		h.done <- Serve(h.ctx, h.bus, ServeOptions{
			...
			Leases:  h.leases, Results: h.results, Clock: h.clock,
			Process: func(ctx context.Context, _ task.Task, _ Options) Outcome {
				h.calls.Add(1)
				h.started <- struct{}{}
...
func (h *harness) publish(attempt int32) task.Task {
	h.t.Helper()
	...
	_, err = h.bus.Publish(h.ctx, events.WorkTranscodeTaskSubject("puid", "cpu", "juid"),
		&events.Envelope{ID: events.MsgIDForTranscodeTask("juid", attempt), Schema: sch, Data: data},
		events.WithMsgID(events.MsgIDForTranscodeTask("juid", attempt)))
	require.NoError(h.t, err)
	return tk
}

func (h *harness) result() task.Result { return h.resultFor(0) }

// resultFor waits for the result of one delivery (0: any), so a retry test
// never reads the previous delivery's result.
func (h *harness) resultFor(delivery uint64) task.Result {
	h.t.Helper()
	var r task.Result
	require.Eventually(h.t, func() bool {
		e, err := h.results.Get(context.Background(), events.TranscodeResultKey("juid"))
		return err == nil && json.Unmarshal(e.Value, &r) == nil && (delivery == 0 || r.Delivery == delivery)
...
func TestServeSucceedsWritesResultReleasesLeaseAndAcks(t *testing.T) {
	h := newHarness(t)
	h.serve()
	h.publish(1)
	<-h.started
	e, err := h.leases.Get(h.ctx, events.TranscodeLeaseKey("juid"))
	require.NoError(t, err, "the lease must be held while Process runs")
	var l task.Lease
	require.NoError(t, json.Unmarshal(e.Value, &l))
	assert.Equal(t, task.LeaseHeld, l.State)
	assert.Equal(t, "pool-abc", l.Pod)

	h.release <- Outcome{Code: ExitOK, Result: &transcodev1alpha1.Result{OutputPath: "/data/x.mkv"}}
	r := h.result()
	assert.True(t, r.Final)
	assert.Equal(t, task.OutcomeSucceeded, r.Outcome)
	assert.Equal(t, "/data/x.mkv", r.Result.OutputPath)
	require.Eventually(t, func() bool {
		_, err := h.leases.Get(h.ctx, events.TranscodeLeaseKey("juid"))
		return err != nil
	}, 5*time.Second, 10*time.Millisecond, "the lease is released after the result")
...
func TestServeReplacesACancelledLeaseFromAnEarlierAttempt(t *testing.T) {
	...
	case <-time.After(5 * time.Second):
		t.Fatal("attempt 2 was dropped by attempt 1's cancel marker")
	}
	h.release <- Outcome{Code: ExitOK, Result: &transcodev1alpha1.Result{}}
	assert.Equal(t, int32(2), h.result().Attempt)
}
...
func TestServeCancelsRunningWorkWhenTheLeaseIsCancelled(t *testing.T) {
	...
	r := h.result()
	assert.True(t, r.Final)
	assert.Equal(t, task.OutcomeCancelled, r.Outcome)
}
...
func TestServeSelfFencesWhenRenewalsFail(t *testing.T) {
	...
	time.Sleep(100 * time.Millisecond)
	_, err := h.results.Get(context.Background(), events.TranscodeResultKey("juid"))
	assert.ErrorIs(t, err, events.ErrKeyNotFound, "a fenced worker reports nothing")
}
...
func TestServeFinishesASucceededTaskDespiteDrain(t *testing.T) {
	...
	assert.Equal(t, task.OutcomeSucceeded, h.result().Outcome)
...
func TestServeDrainNaksUnfinishedWork(t *testing.T) {
	...
	assert.ErrorIs(t, <-h.done, context.Canceled)
	_, err := h.results.Get(context.Background(), events.TranscodeResultKey("juid"))
	assert.Error(t, err, "drained work writes no result")
	_, err = h.leases.Get(context.Background(), events.TranscodeLeaseKey("juid"))
	assert.Error(t, err, "drained work releases its lease so the next pod can start at once")
}

func TestServeRetriesThenExhausts(t *testing.T) {
	h := newHarness(t)
	h.serve()
	h.publish(1)
	for d := 1; d <= 8; d++ {
		<-h.started
		h.release <- Outcome{Code: ExitRetriable, StderrTail: "boom"}
		r := h.resultFor(uint64(d))
		if d < 8 {
			assert.False(t, r.Final, "delivery %d", d)
			assert.Equal(t, task.ReasonRetriable, r.Reason)
			h.clock.Advance(31 * time.Minute) // past the longest backoff
		} else {
			assert.True(t, r.Final)
			assert.Equal(t, task.ReasonRetriesExhausted, r.Reason)
			assert.Equal(t, "boom", r.StderrTail)
		}
	}
}

func TestServeFinalFailuresAck(t *testing.T) {
	for code, reason := range map[int]task.Reason{ExitInvalidSource: task.ReasonInvalidSource, ExitVerifyFailed: task.ReasonVerifyFailed} {
		h := newHarness(t)
		h.serve()
		h.publish(1)
		<-h.started
		h.release <- Outcome{Code: code}
		r := h.result()
		assert.True(t, r.Final)
		assert.Equal(t, reason, r.Reason)
		h.clock.Advance(time.Hour)
		time.Sleep(100 * time.Millisecond)
		assert.Equal(t, int32(1), h.calls.Load(), "a final failure is acked, never retried")
		h.cancel()
	}
}
...
func TestServeEnforcesTheTaskDeadline(t *testing.T) {
	h := newHarness(t)
	h.serve()
	tk := task.Task{Job: schema.Ref{Namespace: "media", Name: "tj", UID: "juid"}, Attempt: 1}
	tk.Deadline.Duration = time.Minute
	sch, data, _ := schema.Encode(tk)
	_, err := h.bus.Publish(h.ctx, events.WorkTranscodeTaskSubject("puid", "cpu", "juid"),
		&events.Envelope{ID: "juid/1", Schema: sch, Data: data})
	require.NoError(t, err)
	<-h.started
	h.clock.Advance(61 * time.Second)
	r := h.result()
	assert.True(t, r.Final)
	assert.Equal(t, task.ReasonDeadlineExceeded, r.Reason)
}
...
- [ ] **Step 4: Write `squasharr/worker/serve.go`**

```go
// Serve pulls this pool's tasks one at a time and runs each to a settled
// message (spec §9). It never returns on its own except for a worker-level
// failure; a cancelled ctx (SIGTERM) returns ctx.Err() once in-flight work is
// drained, and the binary maps that to WorkerExitDrained.
...
func (s *server) handle(ctx context.Context, m events.Message) {
	...
	out := s.o.Process(work, t, s.o.Options)
	cause := context.Cause(work)
	stop(nil)
	<-renewed

	res := task.Result{Job: t.Job, Attempt: t.Attempt, Delivery: m.Attempt(), Pod: s.o.PodName,
		StderrTail: out.StderrTail, StartedAt: started, FinishedAt: s.clock.Now().UTC()}
	if out.Err != nil {
		res.Message = out.Err.Error()
	}
	switch {
	...
	case errors.Is(cause, errCancelled):
		res.Final, res.Outcome, res.Reason = true, task.OutcomeCancelled, task.ReasonCancelled
	case errors.Is(cause, errDeadline):
		res.Final, res.Outcome, res.Reason = true, task.OutcomeFailed, task.ReasonDeadlineExceeded
	...
	case out.Code == ExitOK:
		res.Final, res.Outcome, res.Result = true, task.OutcomeSucceeded, out.Result
	case out.Code == ExitInvalidSource:
		res.Final, res.Outcome, res.Reason = true, task.OutcomeFailed, task.ReasonInvalidSource
	case out.Code == ExitVerifyFailed:
		res.Final, res.Outcome, res.Reason = true, task.OutcomeFailed, task.ReasonVerifyFailed
	case m.Attempt() >= uint64(s.sub.MaxDeliver):
		res.Final, res.Outcome, res.Reason = true, task.OutcomeFailed, task.ReasonRetriesExhausted
	default:
		res.Outcome, res.Reason = task.OutcomeFailed, task.ReasonRetriable
	}
	s.finish(ctx, m, t, rev, res, settle)
}

// finish writes the result, then releases the lease, then settles: a result
// always exists before its message can disappear.
func (s *server) finish(ctx context.Context, m events.Message, t task.Task, rev uint64, res task.Result,
	settle func() (context.Context, context.CancelFunc),
) {
	sctx, cancel := settle()
	defer cancel()
	data, _ := json.Marshal(res)
	if _, err := s.o.Results.Put(sctx, events.TranscodeResultKey(t.Job.UID), data); err != nil {
		logging.FromContext(ctx).WarnContext(ctx, "squasharr worker: result not written; redelivery will redo it", "error", err)
		_ = m.Nak(sctx, 0)
		return
	}
	_ = s.o.Leases.DeleteRevision(sctx, events.TranscodeLeaseKey(t.Job.UID), rev)
	...
	_ = m.Nak(sctx, events.Backoff(s.sub.Backoff, m.Attempt()))
}
```
...
```bash
git add squasharr/worker/serve.go squasharr/worker/lease.go squasharr/worker/serve_test.go squasharr/worker/lease_nats_test.go
git commit -m 'feat(squasharr): the pool worker loop: pull one task, lease, renew with InProgress, settle after the result' -- squasharr/worker/serve.go squasharr/worker/lease.go squasharr/worker/serve_test.go squasharr/worker/lease_nats_test.go
```
...
func run(args []string, getenv func(string) string) int {
	...
	err = worker.Serve(ctx, bus, worker.ServeOptions{
		...
		Leases: bus.KV(events.BucketTranscodeLeases), Results: bus.KV(events.BucketTranscodeResults),
	})
	if ctx.Err() != nil {
...
- Produces:
  ...
  func Classify(stored *batchv1.Job, want corev1.PodTemplateSpec) Drift
  func Mutable(j *batchv1.Job) bool
  type Desired struct { Parallelism int32; Suspend bool }
  func Render(k Key, tp *transcodev1alpha1.TranscodeProfile, want corev1.PodTemplateSpec, d Desired, stored *batchv1.Job, cfg Config) (*batchv1ac.JobApplyConfiguration, error)
...
func rendered(t *testing.T, k Key, tp *transcodev1alpha1.TranscodeProfile, d Desired, stored *batchv1.Job) batchv1.Job {
	t.Helper()
	ac, err := Render(k, tp, Template(tp, k.Class, cfg), d, stored, cfg)
	require.NoError(t, err)
...
func TestRenderIsACompletePoolDeclaration(t *testing.T) {
	...
	assert.Nil(t, j.Spec.Scheduling.SchedulingConstraints, "nothing but minCount under .spec.scheduling")
...
func TestClassifyIgnoresApiserverDefaulting(t *testing.T) { // Review Focus 2
	...
	assert.Equal(t, DriftNone, Classify(&stored, Template(tp, k.Class, cfg)))
}
...
func TestClassify(t *testing.T) {
	...
	assert.Equal(t, DriftReshape, Classify(&stored, Template(reshaped, k.Class, cfg)))
	other := cfg
	other.Image = "transcoder:new"
	assert.Equal(t, DriftRecreate, Classify(&stored, Template(profile(), k.Class, other)))
	delete(stored.Annotations, AnnotationAppliedTemplate)
	assert.Equal(t, DriftRecreate, Classify(&stored, Template(profile(), k.Class, cfg)), "an unknown applied template is recreated")
}
...
func Template(tp *transcodev1alpha1.TranscodeProfile, class transcodev1alpha1.Hardware, cfg Config) corev1.PodTemplateSpec {
	...
	applyHardware(&pod, tp, class, cfg) // job.go:338-366 verbatim: GPU resource, NVIDIA_DRIVER_CAPABILITIES,
	// runtimeClassName, node affinity, intel supplementalGroups, GPU nodeSelector and tolerations
...
// TemplateHash hashes the immutable part of a template.
func TemplateHash(t corev1.PodTemplateSpec) string {
	c := t.DeepCopy()
...
// applied is the template squasharr last applied to j, from its annotation.
// Judging drift against it, not j.Spec.Template, ignores apiserver defaulting.
func applied(j *batchv1.Job) (corev1.PodTemplateSpec, bool) {
	var t corev1.PodTemplateSpec
	raw, ok := j.Annotations[AnnotationAppliedTemplate]
	if !ok || json.Unmarshal([]byte(raw), &t) != nil {
		return t, false
	}
	return t, true
}

// Classify compares what the profile now asks for with what was applied.
func Classify(stored *batchv1.Job, want corev1.PodTemplateSpec) Drift {
	prev, ok := applied(stored)
	if !ok || TemplateHash(prev) != TemplateHash(want) {
		return DriftRecreate
	}
	if !equality.Semantic.DeepEqual(mutablePart(prev), mutablePart(want)) {
		return DriftReshape
	}
	return DriftNone
}
...
// While the Job is not Mutable it re-sends the applied template unchanged:
// a template field this manager stops sending would be released, and a
// released template field on a running Job is a rejected write.
func Render(k Key, tp *transcodev1alpha1.TranscodeProfile, want corev1.PodTemplateSpec, d Desired,
	stored *batchv1.Job, cfg Config,
) (*batchv1ac.JobApplyConfiguration, error) {
	d.Parallelism = max(d.Parallelism, 1)
	tmpl := want
	if stored != nil && !Mutable(stored) {
		prev, ok := applied(stored)
		if !ok {
			return nil, fmt.Errorf("pool %s: no applied template to keep while running", stored.Name)
		}
		tmpl = prev
	}
	...
	job := &batchv1.Job{
		...
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				LabelTemplateHash: TemplateHash(tmpl),
			},
...
func apply(t *testing.T, c client.Client, tp *transcodev1alpha1.TranscodeProfile, d Desired, stored *batchv1.Job) error {
	...
	ac, err := Render(k, tp, Template(tp, "cpu", cfg), d, stored, cfg)
	require.NoError(t, err)
...
func TestPoolLifecycleWithGangScheduling(t *testing.T) {
	...
	stored := get(t, c, tp)
	require.Equal(t, DriftReshape, Classify(stored, Template(tp, "cpu", cfg)))
	...
	assert.Equal(t, DriftNone, Classify(got, Template(tp, "cpu", cfg)))
...
func TestProfileEditWhileRunningNeverRejectsAnApply(t *testing.T) {
	...
	tp.Spec.Resources.Limits[corev1.ResourceCPU] = resource.MustParse("8")
	stored := get(t, c, tp)
	assert.Equal(t, DriftReshape, Classify(stored, Template(tp, "cpu", cfg)))
...
func TestAGateEnabledLaterReadsAsRecreate(t *testing.T) {
	...
	ac, err := Render(Key{Profile: tp.Name, ProfileUID: tp.UID, Class: "cpu"}, tp, Template(tp, "cpu", cfg), Desired{Parallelism: 1}, nil, cfg)
	require.NoError(t, err)
...
func IsSchedulingImmutable(err error) bool {
	return apierrors.IsInvalid(err) && strings.Contains(err.Error(), "spec.scheduling") &&
		strings.Contains(err.Error(), "cannot be set once created")
}
```
...
### Task 10: squasharr dispatches over NATS and is the only writer of status

This is the switch-over. The controller stops creating per-task Jobs. It publishes tasks and reads
lease, result and progress back from KV. The in-process worker role is removed.

**Files:**
- Modify: `squasharr/controller/transcodejob/controller.go`: the new `Reconciler` fields,
  `dispatch`, `observeTask`, admission over TranscodeJobs, and the KV watch source.
- Create: `squasharr/controller/transcodejob/dispatch.go` and `observe.go`, holding the new code
  (keep `controller.go` focused).
...
- Modify: `squasharr/run.go`:
  ...
  - `jobConfig` becomes `poolConfig`, returning a `pool.Config`;
  - `setupControllers` passes the three KV handles.
- Modify: `cmd/clustarr`:
  - remove the `--job` flag (`services.go:255-256`) and the `--role worker` wording
    (`services.go:244`);
  ...
  - move `TestMain`, `runMainEnv`, `runClustarr`, `unreachableKubeconfig` and the RBAC helpers from
    `squasharr_worker_test.go` into a new `cmd/clustarr/helpers_test.go`;
  - delete `TestSquasharrWorkerExitCodeReachesTheProcess` and
    `TestExitCodeOnlyHonoursTheSquasharrWorker`;
  - update `start_envtest_test.go:399-410`'s comment.
- Modify:
  - `squasharr/controller/transcodejob/controller_envtest_test.go`, `events_envtest_test.go` and
    `parity_envtest_test.go`;
  - `importarr/worker/rescan/transcodeoutput_envtest_test.go:50`, which becomes
    `k8s.ManagerSquasharr`.

**Interfaces:**
- Consumes:
  - Task 1: `events.WorkTranscodeTaskSubject`, `MsgIDForTranscodeTask`,
    `TranscodeLeaseKey`/`TranscodeResultKey`, `BucketTranscodeLeases`/`Results`
  - Task 3: `worker.BuildTask`, `task.*`
...
- Produces:
  ...
  type Reconciler struct {
  	...
  	Bus      events.Bus           // publishes tasks and lifecycle events
  	Leases, Results, Progress events.KV
  	Now      func() time.Time
  }
  func poolKey(tj *transcodev1alpha1.TranscodeJob, tp *transcodev1alpha1.TranscodeProfile) pool.Key
  ```

- [ ] **Step 1: Write the failing controller envtests**
...
- It wires `Bus`, `Leases`, `Results` and `Progress` from a `membus.New(nil)` ensured with
  `events.Default().ForSingleNode()`.
- A test worker writes KV entries the way `squasharr/worker/serve.go` does.
...
func (w fakeWorker) lease(attempt int32, state task.LeaseState) {
	b, _ := json.Marshal(task.Lease{Job: schema.Ref{Namespace: "ns", Name: "tj", UID: w.uid}, Attempt: attempt,
		State: state, Pod: "pool-xyz", Since: time.Now()})
	_, err := w.r.Leases.Put(context.Background(), events.TranscodeLeaseKey(w.uid), b)
	require.NoError(w.t, err)
}

func (w fakeWorker) result(res task.Result) {
	res.Job.UID = w.uid
	b, _ := json.Marshal(res)
	_, err := w.r.Results.Put(context.Background(), events.TranscodeResultKey(w.uid), b)
	require.NoError(w.t, err)
}

func (w fakeWorker) progress(percentMilli int32) {
	b, _ := json.Marshal(schema.TranscodeProgress{JobRef: schema.Ref{UID: w.uid}, PercentMilli: percentMilli, BitrateKbps: 900})
	_, err := w.r.Progress.Put(context.Background(), worker.ProgressKey(w.uid), b)
	require.NoError(w.t, err)
}

func TestDispatchStatusAndSingleWriter(t *testing.T) {
	_, c := startEnv(t)
	ns := newNamespace(t, c)
	...
	ps := r.Bus.(events.PullSubscriber)
	p, err := ps.Pull(context.Background(), events.TranscodeTaskConsumer(string(tp.UID), "cpu").Subscription())
	require.NoError(t, err)
	defer p.Stop()
	...
	w := fakeWorker{t: t, r: r, uid: string(tj.UID)}
	w.lease(1, task.LeaseHeld)
	w.progress(42_000)
	reconcileTJ(t, r, tj)
	got = getTJ(t, c, tj)
	assert.Equal(t, transcodev1alpha1.TranscodeJobPhaseRunning, got.Status.Phase)
	assert.Equal(t, "pool-xyz", got.Status.WorkerPod)
	assert.Equal(t, int32(42), got.Status.Progress.Percent)
	assert.Equal(t, int32(900), got.Status.Progress.BitrateKbps)

	w.result(task.Result{Attempt: 1, Final: true, Outcome: task.OutcomeSucceeded, StderrTail: "ok",
		Result: &transcodev1alpha1.Result{OutputPath: "/data/media/Heat.mkv", OutputSizeBytes: 10}})
	reconcileTJ(t, r, tj)
	got = getTJ(t, c, tj)
	assert.Equal(t, transcodev1alpha1.TranscodeJobPhaseSucceeded, got.Status.Phase)
	assert.Equal(t, int64(10), got.Status.Result.OutputSizeBytes)
	assert.Equal(t, "ok", got.Status.StderrTail)
	_, err = r.Results.Get(context.Background(), events.TranscodeResultKey(string(tj.UID)))
	assert.ErrorIs(t, err, events.ErrKeyNotFound, "a recorded result is deleted")

	// One writer: every status leaf is owned by squasharr, on an object in its steady state.
	for mgr := range statusFieldsOf(t, got) { // helper at line 799, returning manager -> fields
		assert.Equal(t, string(k8s.ManagerSquasharr), mgr, "TranscodeJob.status has a second writer")
	}
}

func TestAResultForAnEarlierAttemptIsIgnored(t *testing.T) {
	// Arrange a Queued job with status.attempts = 2 (dispatch, withdraw, dispatch; see Task 12),
	// or apply attempts=2 directly through squasharrstatus.Patch as the test setup.
	// Put a final failed Result{Attempt: 1}; reconcile; the job stays Queued with no failure.
}

func TestNoQueuedWithoutAPublish(t *testing.T) {
	// Wrap r.Bus in a Publisher whose Publish returns events.ErrQueueFull.
	// Reconcile a Planned job: it stays Planned with attempts 0, and the message names the error.
}

func TestASourceUnderNoRootFolderFailsAtDispatch(t *testing.T) {
	// No RootFolder in the namespace: reconcile ends Failed with reason InvalidSource, and nothing is published.
}

func TestALapsedLeaseReturnsTheJobToQueued(t *testing.T) {
	// Running from a held lease; delete the lease key; reconcile: Queued, workerPod cleared.
}
```

Write the four short tests out fully in the same style as `TestDispatchStatusAndSingleWriter`.
Their comments state exactly what each asserts. Where a test needs a failing publisher, define
`type failingPublisher struct{ events.Bus }` with
`Publish(...) (events.Receipt, error) { return events.Receipt{}, events.ErrQueueFull }`.
...
- the Job-completion parts of `TestTransientFailureDoesNotReleaseStatus` and
  `TestTerminalMetricsObservedOnce`: drive those with `fakeWorker.result` instead of
  `completeJob` and `failJob`
- `completeJob`, `failJob` and `getJob`

Keep `TestAdmissionHonoursTheBudget` and `TestAdmissionHonoursProfileMaxConcurrent`, asserting on
dispatched tasks and attempts instead of unsuspended Jobs.

- [ ] **Step 2: Run them to verify they fail**

Run: `go test ./squasharr/controller/transcodejob/ -run 'TestDispatch|TestAResult|TestNoQueued|TestASource|TestALapsed|TestAdmission'`
Expected: FAIL to compile, "unknown field Pool in struct literal".

- [ ] **Step 3: Make status single-writer**

In `squasharr/status/status.go`:
- `ControllerFields` also always sends `StderrTail` and `WorkerPod`, and sends `Progress` (through
  `progressAC`) and `Result` (through `resultAC`) when they are non-nil.
- Delete `WorkerFields` and the `ManagerSquasharrWorker` branch of `Patch`, so `Patch` accepts
  only `k8s.ManagerSquasharr`.
- Rewrite the file comment's two-manager paragraph (lines 41-80): squasharr owns all of
  `TranscodeJob.status`, and the worker reports over NATS (spec §17).

In `split_test.go`:
- delete `TestWorkerFieldsDeclaresExactlyItsOwnSet` and the disjointness test;
- extend the controller field-accounting test so it lists `stderrTail`, `workerPod`, `progress.*`
  and `result.*`.

In `status_envtest_test.go`, delete the worker-manager release cases at lines 131, 168, 270 and
427. Add one case: applying a status with `Progress` and `Result` set, then one where the caller's
seed still carries them, releases nothing.

Delete `ManagerSquasharrWorker` from `pkg/k8s/fieldmanager.go`, from `FieldManagers()` and from
`fieldmanager_test.go`.

- [ ] **Step 4: Write `dispatch.go`**

```go
// poolKey is the (profile, class) pool a job's task goes to. The class comes
// from the plan's encoder (a remux takes a CPU slot), spec §17.2.
func poolKey(tj *transcodev1alpha1.TranscodeJob, tp *transcodev1alpha1.TranscodeProfile) pool.Key {
	return pool.Key{Profile: tp.Name, ProfileUID: tp.UID, Class: hardwareForEncoder(tj.Status.Plan.Encoder)}
}

// dispatch publishes one admitted job's task, then records it: a job is never
// Queued without a task on the queue. It re-reads the job first because the
// admission pass works from a list, not from this job's own reconcile.
func (r *Reconciler) dispatch(ctx context.Context, key types.NamespacedName) error {
	var tj transcodev1alpha1.TranscodeJob
	...
	k := poolKey(&tj, tp)
	t, err := worker.BuildTask(&tj, tp, &mf, folders.Items, attempt, k.Class)
	if errors.Is(err, worker.ErrNoRootFolder) || errors.Is(err, worker.ErrInvalidOutput) {
		r.fail(&tj, desired, string(task.ReasonInvalidSource), "%v", err)
		return r.apply(ctx, &tj, desired)
	}
	if err != nil {
		return err
	}
	...
	if _, err := r.Bus.Publish(ctx, events.WorkTranscodeTaskSubject(string(tp.UID), string(k.Class), string(tj.UID)), env,
		events.WithMsgID(id), events.WithExpectStream(events.StreamWorkSquasharr)); err != nil {
		desired.Message = fmt.Sprintf("dispatch: %v", err)
		_ = r.apply(ctx, &tj, desired)
		return err
	}
	desired.Phase, desired.Attempts = transcodev1alpha1.TranscodeJobPhaseQueued, attempt
	desired.JobRef = ptr.To(pool.Name(k))
	desired.WorkerPod = ""
	desired.Message = fmt.Sprintf("queued for pool %s", pool.Name(k))
	meta.SetStatusCondition(&desired.Conditions, metav1.Condition{Type: transcodev1alpha1.ConditionJobCreated,
		Status: metav1.ConditionTrue, Reason: "Dispatched", Message: desired.Message, ObservedGeneration: tj.Generation})
	return r.apply(ctx, &tj, desired)
}
```

(Use the condition-type constant `transcodev1alpha1` already declares for `JobCreated`, at
`transcodejob_types.go:68-79`.)

- [ ] **Step 5: Write `observe.go`**

```go
// observeTask reads the dispatched job's lease, result and progress (spec
// §17.6). Every read is a Get; the KV watches only wake the reconciler.
func (r *Reconciler) observeTask(ctx context.Context, tj *transcodev1alpha1.TranscodeJob, st *transcodev1alpha1.TranscodeJobStatus) (ctrl.Result, error) {
	uid := string(tj.UID)
	requeue := ctrl.Result{RequeueAfter: worker.DefaultProgressInterval}

	if e, err := r.Results.Get(ctx, events.TranscodeResultKey(uid)); err == nil {
		var res task.Result
		if json.Unmarshal(e.Value, &res) == nil && res.Attempt == st.Attempts {
			st.StderrTail = res.StderrTail
			if !res.Final {
				st.Message = fmt.Sprintf("attempt %d delivery %d failed, retrying: %s", res.Attempt, res.Delivery, res.Message)
			} else {
				r.finishFrom(tj, st, res) // Succeeded/Failed/Skipped + Result, FinishedAt, conditions
				defer func() { _ = r.Results.Delete(ctx, events.TranscodeResultKey(uid)) }() // after the apply below succeeds (see Reconcile)
				return ctrl.Result{}, nil
			}
		}
	} else if !errors.Is(err, events.ErrKeyNotFound) {
		return requeue, err
	}

	e, err := r.Leases.Get(ctx, events.TranscodeLeaseKey(uid))
	var l task.Lease
	switch {
	case err == nil && json.Unmarshal(e.Value, &l) == nil && l.State == task.LeaseHeld && l.Attempt == st.Attempts:
		st.Phase, st.WorkerPod = transcodev1alpha1.TranscodeJobPhaseRunning, l.Pod
		if st.StartedAt == nil {
			st.StartedAt = &metav1.Time{Time: l.Since}
		}
		if p, err := r.Progress.Get(ctx, worker.ProgressKey(uid)); err == nil {
			var tp schema.TranscodeProgress
			if json.Unmarshal(p.Value, &tp) == nil {
				st.Progress = &transcodev1alpha1.Progress{
					Percent: tp.PercentMilli / 1000, Frame: tp.Frame, FPSMilli: tp.FPSCentis * 10,
					SpeedMilli: tp.SpeedCentis * 10, OutTimeMillis: tp.OutTimeMillis, BitrateKbps: tp.BitrateKbps,
					UpdatedAt: metav1.Time{Time: tp.At},
				}
			}
		}
	case err == nil || errors.Is(err, events.ErrKeyNotFound):
		if st.Phase == transcodev1alpha1.TranscodeJobPhaseRunning { // lapsed or released with no final result
			st.Phase, st.WorkerPod = transcodev1alpha1.TranscodeJobPhaseQueued, ""
			st.Message = "the worker's lease lapsed; waiting for redelivery"
		}
	default:
		return requeue, err
	}
...
The result key must be deleted only **after** the terminal status apply has succeeded. So instead
of the `defer` above, return a flag from `observeTask` and have `Reconcile` call
`r.Results.Delete` after `r.apply` returns nil. Structure it that way. The `defer` above only marks
where the delete belongs.

**A dead-lettered task** (spec §13: the worker died on its last delivery, so the MAX_DELIVERIES
watcher dead-lettered the task and no result exists) must fail the job:
- In `catalogarr/history/target.go`, add a resolver for payload schema `transcode.Task.v1` that
  names the TranscodeJob from the payload's `job` ref. Decode only
  `struct{ Job schema.Ref \`json:"job"\` }` so catalogarr does not import `squasharr/task`.
- Add `transcode.Task` beside `transcode.JobEvent` in `pkg/k8s/deadletter.go:58`'s kind table.
- In `observeTask`, as the first check: a Queued or Running job carrying the dead-lettered
  annotation (the one `k8s.MarkDeadLettered` reads) calls
  `r.fail(tj, st, "DeadLettered", "task dead-lettered after %d deliveries", …)`.
- Test `TestADeadLetteredTaskFailsTheJob`: dispatch a job, set the annotation on it, reconcile.
  Expect Failed with reason `DeadLettered`.
- Extend the history package's resolver table test with the new schema.

`finishFrom` maps a final Result onto status:

| Result | Phase | Also sets |
|---|---|---|
| `succeeded` | Succeeded | `Result`, conditions Verified=True (`ReasonWorkerVerified`) and Succeeded=True |
| `skipped` | Skipped | — |
| `failed` / `cancelled` | Failed | the Failed condition, reason = `string(res.Reason)`, message = `res.Message` |

Every row also sets `FinishedAt = res.FinishedAt`.

- [ ] **Step 6: Rewire `Reconcile`, `advance`, `admit` and the watches in `controller.go`**

- **`advance`:**
  - Pending → `plan` (unchanged).
  - Planned → return with `RequeueAfter: requeueQueued`; admission dispatches it.
  - Queued or Running → `observeTask`.
  - Delete `ensureJob`, `observe`, `setSuspend` and `jobSignal`.
- **`admit(ctx)`:**
  - Build `running` from TranscodeJobs in Queued or Running, as
    `Slot{Key: ns/name, Hardware: string(poolKey(...).Class), Profile: tj.Spec.ProfileRef}`.
  - Build `queued` from Planned, non-suspended, non-deleting jobs, with priority and `Created` as
    today.
  - List through `r.reader()` so a job dispatched earlier in this pass is counted.
  - Call `Admit(queued, running, Budget{…})` as today, then `setActive`, then `r.dispatch` for each
    admitted slot.
- **`SetupWithManager`:**
  - Replace `Owns(&batchv1.Job{}, …)` with the KV source:

```go
		WatchesRawSource(source.Func(func(ctx context.Context, q workqueue.TypedRateLimitingInterface[reconcile.Request]) error {
			for _, kv := range []events.KV{r.Leases, r.Results} {
				go r.watchKV(ctx, kv, q)
			}
			return nil
		})).
```
  ...
  - `watchKV`:
    - loops `kv.Watch(ctx, ">")` until `ctx` ends, restarting with a 5-second backoff after an
      error;
    - decodes each entry's value as `struct{ Job schema.Ref \`json:"job"\` }`, the field `Lease`
      and `Result` share;
    - calls `q.Add(reconcile.Request{NamespacedName: types.NamespacedName{Namespace: ref.Namespace, Name: ref.Name}})`;
    - skips deletes and undecodable values.
- **`events.go`:** `transitions` emits `queued` when `st.Attempts > old.Attempts`, not on a
  `JobRef` nil→set edge.

- [ ] **Step 7: Remove the worker role from `squasharr/run.go` and `cmd/clustarr`**

In `squasharr/run.go`:
- Delete `RoleWorker`. `Roles()` returns `[]Role{RoleController}`.
- Delete `runWorker`, `runWorkerJob`, `exitForGet`, `ExitError`, the `JobName` field and the
  worker `Validate` branch.
- `jobConfig` becomes:
...
`setupControllers` sets `Pool: poolConfig(o)`, `Leases: bus.KV(events.BucketTranscodeLeases)`,
`Results: bus.KV(events.BucketTranscodeResults)` and `Progress: bus.KV(events.BucketProgress)`.
Keep `WorkerServiceAccount` for now: Task 13 removes it with its RBAC.

In `cmd/clustarr`:
- delete the `--job` flag;
- delete `jobName` from the `squasharr.Options` literal;
...
- [ ] **Step 8: Update the parity test**

`parity_envtest_test.go`'s `TestStatusPlanIsTheArgvTheWorkerRenders` takes the worker's inputs
from a task built with `worker.BuildTask` from the same objects, and `pool.Threads`. It no longer
reads the Job's `CLUSTARR_CPU_LIMIT`. The planned threads come from `pool.Template`'s env, so read
them with the existing `cpuLimitEnv` helper against `pool.Template(tp, class, cfg)`. It still
asserts `transcode.ArgsHash(workerPlan) == status.plan.argsHash` for HDR10 in place, a container
change and a kept source.

- [ ] **Step 9: Run everything squasharr and cmd touch**

Run: `go test ./squasharr/... ./pkg/k8s/... ./cmd/clustarr/... ./importarr/worker/rescan/...`
Expected: PASS with `KUBEBUILDER_ASSETS` set. Then run `make lint`: expect no new findings and no
forbidigo hits.

- [ ] **Step 10: Commit**

```bash
git add squasharr pkg/k8s/fieldmanager.go pkg/k8s/fieldmanager_test.go pkg/k8s/deadletter.go catalogarr/history cmd/clustarr importarr/worker/rescan/transcodeoutput_envtest_test.go
git commit -m 'feat(squasharr): dispatch tasks over NATS and read status back from KV; one writer of TranscodeJob.status' -- squasharr pkg/k8s/fieldmanager.go pkg/k8s/fieldmanager_test.go pkg/k8s/deadletter.go catalogarr/history cmd/clustarr importarr/worker/rescan/transcodeoutput_envtest_test.go
```
...
**Interfaces:**
- Consumes: `pool.*` (Tasks 8-9), `dispatch` and `poolKey` (Task 10), `k8s.ManagerSquasharrPool`.
...
func TestPoolFollowsDispatch(t *testing.T) {
	...
	for _, tj := range []*transcodev1alpha1.TranscodeJob{a, b} {
		fakeWorker{t: t, r: r, uid: string(tj.UID)}.result(task.Result{Attempt: 1, Final: true,
...
func (r *Reconciler) pools(ctx context.Context, dispatched map[pool.Key]int32) (map[pool.Key]bool, error) {
	...
	for k, n := range keys {
		...
		want := pool.Template(&tp, k.Class, r.Pool)
...
In `admit`:
1. Before calling `Admit`, filter `queued` down to jobs whose `poolKey` is not in the previous
   pass's `draining` set. Held jobs keep `Phase: Planned`, and their message becomes "waiting for
   pool X to drain before its profile change applies".
2. After dispatching, recount `dispatched` per `poolKey` over Queued and Running jobs, call
...
func (r *Reconciler) withdraw(ctx context.Context, tj *transcodev1alpha1.TranscodeJob, tp *transcodev1alpha1.TranscodeProfile) error {
	...
	return r.Admin.PurgeSubject(ctx, events.StreamWorkSquasharr,
		events.WorkTranscodeTaskSubject(string(tp.UID), string(k.Class), uid))
}
```
...
- **Terminal jobs:** remove the finalizer once the result is applied, so a finished job deletes
  instantly.
...
### Task 13: Retire the `squasharr-worker` identity
...
### Task 14: The transcoder image; encoding libraries leave the media image
...
- In the kind config heredoc, add the following. All three gates are shared-registry names the
...
### Task 15: Remove the KEDA transcode example; rewrite scenario 12 for pools
...
### Task 16: Documents: ADR-0009 accepted, design spec as built, CLAUDE.md
...
- `docs/superpowers/specs/2026-09-18-clustarr-design.md`:
  ...
  - §6.4 (lines 672-676): rewrite as built. Pools per (profile, class), dispatch, the lease
    protocol, squasharr as the single status writer, the two images, and the exit codes. Cite
    spec 2026-09-23 §17.
...
- `config/keda/README.md`, if Task 15 left anything citing the slot scheduler as an alternative.
